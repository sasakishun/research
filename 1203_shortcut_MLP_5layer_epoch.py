...
class layer:
    ...
    def prepare_model(self):
        ...
        with tf.name_scope('layer3'):
            with tf.name_scope('w2'):
                w3 = tf.Variable(
                    tf.truncated_normal(shape=[input_size, num_units3], mean=0.0, stddev=math.sqrt(1 / input_size)))
        ...
        with tf.name_scope('layer2'):
            with tf.name_scope('w2'):
                w2 = tf.Variable(
                    tf.truncated_normal(shape=[num_units3, num_units2], mean=0.0, stddev=math.sqrt(1 / num_units3)))
            with tf.name_scope('b2'):
        ...
        with tf.name_scope('layer0'):
            ...
            with tf.name_scope('shortcut_weight'):
                ...
                w0_3 = tf.Variable(
                    tf.truncated_normal(shape=[num_units3, output_size], mean=0.0, stddev=math.sqrt(1 / num_units3)))
                w0_2 = tf.Variable(
        ...
        with tf.name_scope('optimizer'):
            ...
            with tf.name_scope('train_step'):
                # train_step = tf.train.AdamOptimizer().minimize(loss, var_list=[w0, b0, w1, b1, w2, b2, w3, b3])
    ...
    def prepare_session(self):
        ...
        writer = tf.summary.FileWriter("./log/1203_shortcut_MLP_epoch", sess.graph)
...
if __name__ == '__main__':
    ...
    session_name = "./session_log/saver_1203_shortcut_MLP_5layer_epoch_5"
