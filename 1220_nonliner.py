...
class layer:
    ...
    def prepare_model(self):
        ...
        with tf.name_scope('layer1'):
            ...
            with tf.name_scope('b1'):
                ...
                b1_8 = tf.Variable(tf.constant(0.1, shape=[num_units1]))
                b1_7 = tf.Variable(tf.constant(0.1, shape=[num_units1]))
                b1_6 = tf.Variable(tf.constant(0.1, shape=[num_units1]))
                b1_5 = tf.Variable(tf.constant(0.1, shape=[num_units1]))
                b1_4 = tf.Variable(tf.constant(0.1, shape=[num_units1]))
                b1_3 = tf.Variable(tf.constant(0.1, shape=[num_units1]))
                b1_2 = tf.Variable(tf.constant(0.1, shape=[num_units1]))
        ...
        with tf.name_scope('feed_forword'):
            hidden_8 = tf.layers.batch_normalization(
                tf.nn.relu(tf.layers.batch_normalization(tf.matmul(x, w8) + b8, training=training)), training=training)
            hidden_7 = tf.layers.batch_normalization(
                tf.nn.relu(tf.layers.batch_normalization(tf.matmul(hidden_8, w7) + b7, training=training)),
                training=training)
            hidden_6 = tf.layers.batch_normalization(
                tf.nn.relu(tf.layers.batch_normalization(tf.matmul(hidden_7, w6) + b6, training=training)),
                training=training)
            hidden_5 = tf.layers.batch_normalization(
                tf.nn.relu(tf.layers.batch_normalization(tf.matmul(hidden_6, w5) + b5, training=training)),
                training=training)
            hidden_4 = tf.layers.batch_normalization(
                tf.nn.relu(tf.layers.batch_normalization(tf.matmul(hidden_5, w4) + b4, training=training)),
                training=training)
            hidden_3 = tf.layers.batch_normalization(
                tf.nn.relu(tf.layers.batch_normalization(tf.matmul(hidden_4, w3) + b3, training=training)),
                training=training)
            hidden_2 = tf.layers.batch_normalization(
                tf.nn.relu(tf.layers.batch_normalization(tf.matmul(hidden_3, w2) + b2, training=training)),
                training=training)
            """
            hidden_8 = tf.nn.relu(tf.matmul(tf.layers.batch_normalization(x, training=training), w8) + b8)
            hidden_7 = tf.nn.relu(tf.matmul(tf.layers.batch_normalization(hidden_8, training=training), w7) + b7)
            hidden_6 = tf.nn.relu(tf.matmul(tf.layers.batch_normalization(hidden_7, training=training), w6) + b6)
            hidden_5 = tf.nn.relu(tf.matmul(tf.layers.batch_normalization(hidden_6, training=training), w5) + b5)
            hidden_4 = tf.nn.relu(tf.matmul(tf.layers.batch_normalization(hidden_5, training=training), w4) + b4)
            hidden_3 = tf.nn.relu(tf.matmul(tf.layers.batch_normalization(hidden_4, training=training), w3) + b3)
            hidden_2 = tf.nn.relu(tf.matmul(tf.layers.batch_normalization(hidden_3, training=training), w2) + b2)
            """

            """
            hidden_1_8 = tf.nn.relu((tf.layers.batch_normalization(hidden_8, training=training) + b1))
            p0 = tf.nn.softmax(tf.layers.batch_normalization(tf.matmul(hidden_1_8, w0_8)) + b0)

            hidden_1_7 = tf.nn.relu((tf.layers.batch_normalization(hidden_8 +
                                                                   tf.layers.batch_normalization(hidden_7, training=training),
                                                                   training=training)/2 + b1))
            p1 = tf.nn.softmax(tf.layers.batch_normalization(tf.matmul(hidden_1_7, w0_7)) + b0)

            hidden_1_6 = tf.nn.relu((tf.layers.batch_normalization(hidden_8 +
                                                                   tf.layers.batch_normalization(hidden_6, training=training),
                                                                   training=training)/2 + b1))
            p2 = tf.nn.softmax(tf.layers.batch_normalization(tf.matmul(hidden_1_6, w0_6)) + b0)

            hidden_1_5 = tf.nn.relu((tf.layers.batch_normalization(hidden_8 +
                                                                   tf.layers.batch_normalization(hidden_5, training=training),
                                                                   training=training)/2 + b1))
            p3 = tf.nn.softmax(tf.layers.batch_normalization(tf.matmul(hidden_1_5, w0_5)) + b0)

            hidden_1_4 = tf.nn.relu((tf.layers.batch_normalization(hidden_8 +
                                                                   tf.layers.batch_normalization(hidden_4, training=training),
                                                                   training=training)/2 + b1))
            p4 = tf.nn.softmax(tf.layers.batch_normalization(tf.matmul(hidden_1_4, w0_4)) + b0)

            hidden_1_3 = tf.nn.relu((tf.layers.batch_normalization(hidden_8 +
                                                                   tf.layers.batch_normalization(hidden_3, training=training),
                                                                   training=training)/2 + b1))
            p5 = tf.nn.softmax(tf.layers.batch_normalization(tf.matmul(hidden_1_3, w0_3)) + b0)

            hidden_1_2 = tf.nn.relu((tf.layers.batch_normalization(hidden_8 +
                                                                   tf.layers.batch_normalization(hidden_2, training=training),
                                                                   training=training)/2 + b1))
            p6 = tf.nn.softmax(tf.layers.batch_normalization(tf.matmul(hidden_1_2, w0_2)) + b0)

            """
            hidden_1_8 = tf.nn.relu((tf.layers.batch_normalization(hidden_8 + b1_8, training=training)))
            p0 = tf.nn.softmax(tf.layers.batch_normalization(tf.matmul(hidden_1_8, w0_8) + b0, training=training))

            hidden_1_7 = tf.nn.relu(
                tf.layers.batch_normalization((hidden_8 + hidden_7
                                               ) / 2 + b1_7, training=training))
            p1 = tf.nn.softmax(tf.layers.batch_normalization(tf.matmul(hidden_1_7, w0_7) + b0, training=training))

            hidden_1_6 = tf.nn.relu(
                tf.layers.batch_normalization((hidden_8 + hidden_7 + hidden_6
                                               ) / 3 + b1_6, training=training))
            p2 = tf.nn.softmax(tf.layers.batch_normalization(tf.matmul(hidden_1_6, w0_6) + b0, training=training))

            hidden_1_5 = tf.nn.relu(
                tf.layers.batch_normalization((hidden_8 + hidden_7 + hidden_6 + hidden_5
                                               ) / 4 + b1_5, training=training))
            p3 = tf.nn.softmax(tf.layers.batch_normalization(tf.matmul(hidden_1_5, w0_5) + b0, training=training))

            hidden_1_4 = tf.nn.relu(
                (tf.layers.batch_normalization(
                    (tf.layers.batch_normalization(
                        hidden_8 + hidden_7 + hidden_6 + hidden_5 + hidden_4,
                        training=training)
                    ) / 1 + b1_4, training=training)))
            p4 = tf.nn.softmax(tf.layers.batch_normalization(tf.matmul(hidden_1_4, w0_4) + b0, training=training))

            hidden_1_3 = tf.nn.relu(
                (tf.layers.batch_normalization(
                    (tf.layers.batch_normalization(
                        hidden_8 + hidden_7 + hidden_6 + hidden_5 + hidden_4 + hidden_3,
                        training=training)
                    ) / 1 + b1_3, training=training)))
            p5 = tf.nn.softmax(tf.layers.batch_normalization(tf.matmul(hidden_1_3, w0_3) + b0, training=training))

            hidden_1_2 = tf.nn.relu((tf.layers.batch_normalization(
                (hidden_8 + hidden_7 + hidden_6 + hidden_5 + hidden_4 + hidden_3 + hidden_2) / 1 + b1_2,
                training=training)))
            p6 = tf.nn.softmax(tf.layers.batch_normalization(tf.matmul(hidden_1_2, w0_2) + b0, training=training))
            # p0 = tf.nn.softmax(tf.matmul(tf.layers.batch_normalization(hidden_1_8, training=training), w0_8) + b0_8)
            # p1 = tf.nn.softmax(tf.matmul(tf.layers.batch_normalization(hidden_1_7, training=training), w0_7) + b0_7)
            # p2 = tf.nn.softmax(tf.matmul(tf.layers.batch_normalization(hidden_1_6, training=training), w0_6) + b0_6)
            # p3 = tf.nn.softmax(tf.matmul(tf.layers.batch_normalization(hidden_1_5, training=training), w0_5) + b0_5)
            # p4 = tf.nn.softmax(tf.matmul(tf.layers.batch_normalization(hidden_1_4, training=training), w0_4) + b0_4)
            # p5 = tf.nn.softmax(tf.matmul(tf.layers.batch_normalization(hidden_1_3, training=training), w0_3) + b0_3)
            # p6 = tf.nn.softmax(tf.matmul(tf.layers.batch_normalization(hidden_1_2, training=training), w0_2) + b0_2)
        ...
        with tf.name_scope('train_step0'):
            ...
            with tf.control_dependencies(update_ops):
                """
                train_step0 = tf.train.AdamOptimizer().minimize(loss0, var_list=[w0, b0, w8, b8])
                train_step1 = tf.train.AdamOptimizer().minimize(loss1, var_list=[w0, b0, w7, b7])
                train_step2 = tf.train.AdamOptimizer().minimize(loss2, var_list=[w0, b0, w6, b6])
                train_step3 = tf.train.AdamOptimizer().minimize(loss3, var_list=[w0, b0, w5, b5])
                train_step4 = tf.train.AdamOptimizer().minimize(loss4, var_list=[w0, b0, w4, b4])
                train_step5 = tf.train.AdamOptimizer().minimize(loss5, var_list=[w0, b0, w3, b3])
                train_step6 = tf.train.AdamOptimizer().minimize(loss6, var_list=[w0, b0, w2, b2])
                """
                train_step0 = tf.train.AdamOptimizer().minimize(loss0, var_list=[w0_8, b0, b1_8, w8, b8])
                train_step1 = tf.train.AdamOptimizer().minimize(loss1, var_list=[w0_7, b0, b1_7, w7, b7])
                train_step2 = tf.train.AdamOptimizer().minimize(loss2, var_list=[w0_6, b0, b1_6, w6, b6])
                train_step3 = tf.train.AdamOptimizer().minimize(loss3, var_list=[w0_5, b0, b1_5, w5, b5])
                train_step4 = tf.train.AdamOptimizer().minimize(loss4, var_list=[w0_4, b0, b1_4, w4, b4])
                train_step5 = tf.train.AdamOptimizer().minimize(loss5, var_list=[w0_3, b0, b1_3, w3, b3])
                train_step6 = tf.train.AdamOptimizer().minimize(loss6, var_list=[w0_2, b0, b1_2, w2, b2])
                """
                train_step0 = tf.train.AdamOptimizer().minimize(loss0, var_list=[b0, w0_9])
                train_step1 = tf.train.AdamOptimizer().minimize(loss1, var_list=[b0, w8, b8, w0_8])
                train_step2 = tf.train.AdamOptimizer().minimize(loss2,
                                                                var_list=[b0, w8, b8, w7, b7, w0_7])
                train_step3 = tf.train.AdamOptimizer().minimize(loss3,
                                                                var_list=[b0, w8, b8, w7, b7, w6, b6, w0_6])
                train_step4 = tf.train.AdamOptimizer().minimize(loss4,
                                                                var_list=[b0, w8, b8, w7, b7, w6, b6, w5, b5, w0_5])
                train_step5 = tf.train.AdamOptimizer().minimize(loss5,
                                                                var_list=[b0, w8, b8, w7, b7, w6, b6, w5, b5, w4, b4, w0_4])
                train_step6 = tf.train.AdamOptimizer().minimize(loss6,
                                                                var_list=[b0, w8, b8, w7, b7, w6, b6, w5, b5, w4, b4, w3, b3, w0_3])
                train_step7 = tf.train.AdamOptimizer().minimize(loss7,
                                                                var_list=[b0, w8, b8, w7, b7, w6, b6, w5, b5, w4, b4, w3, b3, w2, b2, w0_2])
                train_step8 = tf.train.AdamOptimizer().minimize(loss8,
                                                                var_list=[b0, w8, b8, w7, b7, w6, b6, w5, b5, w4, b4, w3, b3, w2, b2, w1, b1, w0])
                """
                """
                train_step0 = tf.train.AdamOptimizer().minimize(loss0, var_list=[b0, w0_9])
                train_step1 = tf.train.AdamOptimizer().minimize(loss1, var_list=[b0, w8, b8, gate_9, w0_8])
                train_step2 = tf.train.AdamOptimizer().minimize(loss2,
                                                                var_list=[b0, w8, b8, w7, b7, gate_8, w0_7])
                train_step3 = tf.train.AdamOptimizer().minimize(loss3,
                                                                var_list=[b0, w8, b8, w7, b7, w6, b6, gate_7, w0_6])
                train_step4 = tf.train.AdamOptimizer().minimize(loss4,
                                                                var_list=[b0, w8, b8, w7, b7, w6, b6, w5, b5,
                                                                          gate_6, w0_5])
                train_step5 = tf.train.AdamOptimizer().minimize(loss5,
                                                                var_list=[b0, w8, b8, w7, b7, w6, b6, w5, b5, w4,
                                                                          b4, gate_5, w0_4])
                train_step6 = tf.train.AdamOptimizer().minimize(loss6,
                                                                var_list=[b0, w8, b8, w7, b7, w6, b6, w5, b5, w4,
                                                                          b4, w3, b3, gate_4, w0_3])
                """
        # Adam
        """
        with tf.name_scope('train_step0'):
        ...
        tf.summary.scalar("hidden1_8", tf.reduce_sum(hidden_1_8))
        tf.summary.scalar("hidden1_7", tf.reduce_sum(hidden_1_7))
        tf.summary.scalar("hidden1_6", tf.reduce_sum(hidden_1_6))
        tf.summary.scalar("hidden1_5", tf.reduce_sum(hidden_1_5))
        tf.summary.scalar("hidden1_4", tf.reduce_sum(hidden_1_4))
        tf.summary.scalar("hidden1_3", tf.reduce_sum(hidden_1_3))
        tf.summary.scalar("hidden1_2", tf.reduce_sum(hidden_1_2))
    ...
    def prepare_session(self):
        ...
        writer = tf.summary.FileWriter("./log/1220_nonliner", sess.graph)
...
if __name__ == '__main__':
    ...
    drop_out_rate = 1.0
    session_name = "./session_log/saver_1220_nonliner_5"
    training = True
    testing = False
    ...
    final_loss_layer = 0
    graph_x = range(1000)
    graph_y = [[None for i in range(1000)] for j in range(layer_num + 1)]
    graph_train_y = [[None for i in range(1000)] for j in range(layer_num + 1)]
    graph_valid_y = [[None for i in range(1000)] for j in range(layer_num + 1)]
    ...
    for j in range(start, layer_num + 1):
        ...
        for i in range(loop_len):
            ...
            if i % 1 == 0:
                ...
                graph_valid_y[j][loop_count] = acc_val
                # test_data
                summary, loss_val, acc_val = nn.sess.run(
                    [nn.summary, nn.loss[j], nn.accuracy[j]],
                    feed_dict={nn.x: test_data, nn.t: test_labels, nn.keep_prob: 1.0, nn.training: testing,
        ...
        for reset_num in range(early_stopping_num):
            ...
            graph_valid_y[j][loop_count + 1 + reset_num] = None
        if j < layer_num:
            graph_y[j + 1][loop_count] = acc_val
            graph_train_y[j + 1][loop_count] = acc_val_train
            graph_valid_y[j + 1][loop_count] = acc_val_valid
        j += 1
        loop_count += 1
    ...
    for i in range(layer_num + 1):
        ax.plot(graph_x, graph_valid_y[-i], linewidth=1.0, marker=".", markersize=10, linestyle="dashed")
    ax.grid(which="both")
    ax.set_xlabel("epoch")
    ax.set_ylabel("accuracy")
    plt.title(str(train_size) + "images:nonliner")
    ax.set_xlim([0, loop_count - 1])

    np.save("nonliner_graph_y", graph_y)
    np.save("graph_x", graph_x)

    plt.show()

    data_MLP = np.load("MLP_graph_y.npy")

    ax = plt.subplot()
    for i in range(layer_num + 1):
        ax.plot(graph_x, graph_y[-i], linewidth=1.0, marker=",", markersize=10)
    for i in range(layer_num + 1):
        ax.plot(graph_x, data_MLP[-i], linewidth=1.0, marker=".", markersize=10)
    ax.grid(which="both")
    ax.set_xlabel("epoch")
    ax.set_ylabel("accuracy")
    plt.title(str(train_size) + "images")
    ax.set_xlim([0, loop_count - 1])
    plt.show()
