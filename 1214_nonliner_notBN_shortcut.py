...
class layer:
    ...
    def prepare_model(self):
        ...
        with tf.name_scope('feed_forword'):
            ...
            with tf.name_scope('p1'):
                hidden_1_7 = tf.nn.relu(((hidden_8 + tf.layers.batch_normalization(hidden_7, training=training)) / 2 + b1))
                p1 = tf.nn.softmax(tf.matmul(hidden_1_7, w0) + b0)
            with tf.name_scope('p2'):
                hidden_1_6 = tf.nn.relu(((hidden_8 + hidden_7 +
                                         tf.layers.batch_normalization(hidden_6,training=training)) / 3 + b1))
                p2 = tf.nn.softmax(tf.matmul(hidden_1_6, w0) + b0)
            with tf.name_scope('p3'):
                hidden_1_5 = tf.nn.relu(((hidden_8 + hidden_7 + hidden_6 +
                                         tf.layers.batch_normalization(hidden_5,
                                                                       training=training)) / 4 + b1))
                p3 = tf.nn.softmax(tf.matmul(hidden_1_5, w0) + b0)
            with tf.name_scope('p4'):
                hidden_1_4 = tf.nn.relu((
                    (hidden_8 + hidden_7 + hidden_6 + hidden_5 + tf.layers.batch_normalization(hidden_4,
                                                                                               training=training)) / 5 + b1))
                p4 = tf.nn.softmax(tf.matmul(hidden_1_4, w0) + b0)
            with tf.name_scope('p5'):
                hidden_1_3 = tf.nn.relu(
                    ((hidden_8 + hidden_7 + hidden_6 + hidden_5 + hidden_4 + tf.layers.batch_normalization(hidden_3,
                                                                                                          training=training)) / 6 + b1))
                p5 = tf.nn.softmax(tf.matmul(hidden_1_3, w0) + b0)
            with tf.name_scope('p6'):
                hidden_1_2 = tf.nn.relu((hidden_8 + hidden_7 + hidden_6 + hidden_5 + hidden_4 + hidden_3 + (
                    tf.layers.batch_normalization(hidden_2,
    ...
    def prepare_session(self):
        ...
        writer = tf.summary.FileWriter("./log/1214_nonliner_notBN_shortcut", sess.graph)
...
if __name__ == '__main__':
    ...
    session_name = "./session_log/saver_1214_nonliner_notBN_shortcut_3"
    training = True
    testing = False
