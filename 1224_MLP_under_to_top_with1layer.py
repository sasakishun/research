...
class layer:
    ...
    def prepare_model(self):
        ...
        with tf.name_scope('layer_inner_shortcut'):
            ...
            with tf.name_scope('b0'):
                b8_inner = tf.Variable(tf.constant(0.1, shape=[num_units_inner]))
                b7_inner = tf.Variable(tf.constant(0.1, shape=[num_units_inner]))
                b6_inner = tf.Variable(tf.constant(0.1, shape=[num_units_inner]))
                b5_inner = tf.Variable(tf.constant(0.1, shape=[num_units_inner]))
                b4_inner = tf.Variable(tf.constant(0.1, shape=[num_units_inner]))
                b3_inner = tf.Variable(tf.constant(0.1, shape=[num_units_inner]))
                b2_inner = tf.Variable(tf.constant(0.1, shape=[num_units_inner]))
        ...
        with tf.name_scope('feed_forword'):
            ...
            p0 = tf.nn.softmax(tf.layers.batch_normalization(tf.matmul(hidden_8, w0_8), training=training) + b0_8)
            p1 = tf.nn.softmax(tf.layers.batch_normalization(tf.matmul(hidden_7, w0_7), training=training) + b0_7)
            p2 = tf.nn.softmax(tf.layers.batch_normalization(tf.matmul(hidden_6, w0_6), training=training) + b0_6)
            p3 = tf.nn.softmax(tf.layers.batch_normalization(tf.matmul(hidden_5, w0_5), training=training) + b0_5)
            p4 = tf.nn.softmax(tf.layers.batch_normalization(tf.matmul(hidden_4, w0_4), training=training) + b0_4)
            p5 = tf.nn.softmax(tf.layers.batch_normalization(tf.matmul(hidden_3, w0_3), training=training) + b0_3)
            p6 = tf.nn.softmax(tf.layers.batch_normalization(tf.matmul(hidden_2, w0_2), training=training) + b0_2)
            """
            ...
            """
            """
            p0 = tf.nn.softmax(tf.layers.batch_normalization(tf.matmul(hidden_8, w0_8) + b0_8, training=training))
            p1 = tf.nn.softmax(tf.layers.batch_normalization(tf.matmul(hidden_7, w0_7) + b0_7, training=training))
            p2 = tf.nn.softmax(tf.layers.batch_normalization(tf.matmul(hidden_6, w0_6) + b0_6, training=training))
            p3 = tf.nn.softmax(tf.layers.batch_normalization(tf.matmul(hidden_5, w0_5) + b0_5, training=training))
            p4 = tf.nn.softmax(tf.layers.batch_normalization(tf.matmul(hidden_4, w0_4) + b0_4, training=training))
            p5 = tf.nn.softmax(tf.layers.batch_normalization(tf.matmul(hidden_3, w0_3) + b0_3, training=training))
            p6 = tf.nn.softmax(tf.layers.batch_normalization(tf.matmul(hidden_2, w0_2) + b0_2, training=training))
            """

            hidden_inner_8 = tf.nn.relu(tf.layers.batch_normalization(tf.matmul(hidden_8, w_inner_8) + b8_inner, training=training))
            hidden_inner_7 = tf.nn.relu(tf.layers.batch_normalization(tf.matmul(hidden_7, w_inner_7) + b7_inner, training=training))
            hidden_inner_6 = tf.nn.relu(tf.layers.batch_normalization(tf.matmul(hidden_6, w_inner_6) + b6_inner, training=training))
            hidden_inner_5 = tf.nn.relu(tf.layers.batch_normalization(tf.matmul(hidden_5, w_inner_5) + b5_inner, training=training))
            hidden_inner_4 = tf.nn.relu(tf.layers.batch_normalization(tf.matmul(hidden_4, w_inner_4) + b4_inner, training=training))
            hidden_inner_3 = tf.nn.relu(tf.layers.batch_normalization(tf.matmul(hidden_3, w_inner_3) + b3_inner, training=training))
            hidden_inner_2 = tf.nn.relu(tf.layers.batch_normalization(tf.matmul(hidden_2, w_inner_2) + b2_inner, training=training))

            p0 = tf.nn.softmax(tf.matmul(hidden_inner_8, w0_inner_8) + b0_8)
            p1 = tf.nn.softmax(tf.matmul(hidden_inner_7, w0_inner_7) + b0_7)
            p2 = tf.nn.softmax(tf.matmul(hidden_6, w0_6) + b0_6)
            p3 = tf.nn.softmax(tf.matmul(hidden_6, w0_6) + b0_6)
        ...
        with tf.name_scope('train_step0'):
            ...
            with tf.control_dependencies(update_ops):
                """
                train_step0 = tf.train.AdamOptimizer().minimize(loss0, var_list=[w8, b8])
                train_step1 = tf.train.AdamOptimizer().minimize(loss1, var_list=[w7, b7])
                train_step2 = tf.train.AdamOptimizer().minimize(loss2, var_list=[w6, b6])
                train_step3 = tf.train.AdamOptimizer().minimize(loss2, var_list=[w0_6, b0_6])
                train_step4 = tf.train.AdamOptimizer().minimize(loss4, var_list=[w0_4, b0_4, w4, b4])
                train_step5 = tf.train.AdamOptimizer().minimize(loss5, var_list=[w0_3, b0_3, w3, b3])
                train_step6 = tf.train.AdamOptimizer().minimize(loss6, var_list=[w0_2, b0_2, w2, b2])
                """
                train_step0 = tf.train.AdamOptimizer().minimize(loss0, var_list=[w_inner_8, b8_inner, w0_inner_8, b0_8, w8, b8])

                train_step1 = tf.train.AdamOptimizer().minimize(loss1, var_list=[w_inner_7, b7_inner, w0_inner_7, b0_7, w7, b7])

                train_step2 = tf.train.AdamOptimizer().minimize(loss2, var_list=[w0_6, b0_6, w6, b6])
                train_step3 = tf.train.AdamOptimizer().minimize(loss3, var_list=[w0_6, b0_6])
                """
                train_step0 = tf.train.AdamOptimizer().minimize(loss0, var_list=[w_inner_8, b8_inner, w0_inner_8, b0_8, w8, b8])
                train_step1 = tf.train.AdamOptimizer().minimize(loss1, var_list=[w_inner_7, b7_inner, w0_inner_7, b0_7, w7, b7])
                train_step2 = tf.train.AdamOptimizer().minimize(loss2, var_list=[w0_6, b0_6, w6, b6])
                train_step3 = tf.train.AdamOptimizer().minimize(loss3, var_list=[w0_6, b0_6])
                """
                # train_step2 = tf.train.AdamOptimizer().minimize(loss2, var_list=[w_inner_6, b6_inner, w0_inner_6, b0_6, w6,b6])
                # train_step3 = tf.train.AdamOptimizer().minimize(loss3, var_list=[w_inner_5, b5_inner, w0_inner_5, b0_5, w5, b5])
                train_step4 = tf.train.AdamOptimizer().minimize(loss4, var_list=[w_inner_4, b4_inner, w0_inner_4, b0_4, w4, b4])
                train_step5 = tf.train.AdamOptimizer().minimize(loss5, var_list=[w_inner_3, b3_inner, w0_inner_3, b0_3, w3, b3])
                train_step6 = tf.train.AdamOptimizer().minimize(loss6, var_list=[w_inner_2, b2_inner, w0_inner_2, b0_2, w2, b2])
...
if __name__ == '__main__':
    ...
    session_name = "./session_log/saver" + path.splitext(path.basename(__file__))[0] + "_20"
    print(session_name)
    training = True
    testing = False
