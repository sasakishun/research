...
class layer:
    ...
    def prepare_model(self):
        ...
        with tf.name_scope('layer0'):
            ...
            with tf.name_scope('w0_9'):
                w0_9 = tf.Variable(
                    tf.truncated_normal(shape=[input_size, output_size], mean=0.0, stddev=math.sqrt(2 / input_size)))
            with tf.name_scope('w7_9'):
                w0_8 = tf.Variable(
                    tf.truncated_normal(shape=[num_units8, output_size], mean=0.0, stddev=math.sqrt(2 / num_units8)))
            with tf.name_scope('w7_9'):
                w0_7 = tf.Variable(
                    tf.truncated_normal(shape=[num_units7, output_size], mean=0.0, stddev=math.sqrt(2 / num_units7)))
            with tf.name_scope('w6_9'):
                w0_6 = tf.Variable(
                    tf.truncated_normal(shape=[num_units6, output_size], mean=0.0, stddev=math.sqrt(2 / num_units6)))
            """
            ...
            with tf.name_scope('w0_9'):
                w0_9 = tf.Variable(
                    tf.truncated_normal(shape=[input_size, output_size], mean=0.0,
                                        stddev=math.sqrt(1 / (input_size + num_units8 + num_units7 + num_units6))))
            with tf.name_scope('w7_9'):
                w0_8 = tf.Variable(
                    tf.truncated_normal(shape=[num_units8, output_size], mean=0.0,
                                        stddev=math.sqrt(1 / (input_size + num_units8 + num_units7 + num_units6))))
            with tf.name_scope('w7_9'):
                w0_7 = tf.Variable(
                    tf.truncated_normal(shape=[num_units7, output_size], mean=0.0,
                                        stddev=math.sqrt(1 / (input_size + num_units8 + num_units7 + num_units6))))
            with tf.name_scope('w6_9'):
                w0_6 = tf.Variable(
                    tf.truncated_normal(shape=[num_units6, output_size], mean=0.0,
                                        stddev=math.sqrt(1 / (input_size + num_units8 + num_units7 + num_units6))))
            """
        ...
        with tf.name_scope('feed_forword'):
            ...
            with tf.name_scope('state1'):
                ...
                with tf.name_scope('p1'):
                    p1 = tf.nn.softmax(tf.matmul(hidden1_8_drop, w0_8)
                                       + b0
                                       + tf.matmul(x, w0_9))
            ...
            with tf.name_scope('state2'):
                ...
                with tf.name_scope('p2'):
                    p2 = tf.nn.softmax(tf.matmul(hidden2_7_drop, w0_7)
                                       + b0
                                       + tf.matmul(x, w0_9)
                                       + tf.matmul(hidden2_8, w0_8))
            ...
            with tf.name_scope('state3'):
                ...
                with tf.name_scope('p3'):
                    p3 = tf.nn.softmax(tf.matmul(x, w0_9)
                                       + b0
            ...
            with tf.name_scope('state4'):
                ...
                with tf.name_scope('p4'):
                    p4 = tf.nn.softmax(tf.matmul(x, w0_9)
                                       + b0
    ...
    def prepare_session(self):
        ...
        writer = tf.summary.FileWriter("./log/1206_MNIST_shortcut_MLP", sess.graph)
...
if __name__ == '__main__':
    ...
    session_name = "./session_log/saver_1206_MNIST_shortcut_MLP_7"
