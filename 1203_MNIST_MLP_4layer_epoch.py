...
class layer:
    ...
    def prepare_model(self):
        ...
        with tf.name_scope('layer8'):
            with tf.name_scope('w8'):
                w8 = tf.Variable(tf.truncated_normal(shape=[input_size, num_units8], mean=0.0, stddev=math.sqrt(1 / input_size)))
            with tf.name_scope('b8'):
                b8 = tf.Variable(tf.constant(0.1, shape=[num_units8]))

        with tf.name_scope('layer7'):
            with tf.name_scope('w7'):
                w7 = tf.Variable(tf.truncated_normal(shape=[num_units8, num_units7], mean=0.0, stddev=math.sqrt(1 / num_units8)))
            with tf.name_scope('b7'):
                b7 = tf.Variable(tf.constant(0.1, shape=[num_units7]))

        with tf.name_scope('layer6'):
            with tf.name_scope('w6'):
                w6 = tf.Variable(tf.truncated_normal(shape=[num_units7, num_units6], mean=0.0, stddev=math.sqrt(1 / num_units7)))
        ...
        with tf.name_scope('layer0'):
            ...
            with tf.name_scope('w7_9'):
                w0_8 = tf.Variable(
                    tf.truncated_normal(shape=[num_units8, output_size], mean=0.0, stddev=math.sqrt(1 / num_units8)))
            with tf.name_scope('w7_9'):
                w0_7 = tf.Variable(
                    tf.truncated_normal(shape=[num_units7, output_size], mean=0.0, stddev=math.sqrt(1 / num_units7)))
            with tf.name_scope('w6_9'):
                w0_6 = tf.Variable(
                    tf.truncated_normal(shape=[num_units6, output_size], mean=0.0, stddev=math.sqrt(1 / num_units6)))
        ...
        with tf.name_scope('feed_forword'):
            ...
            with tf.name_scope('state4'):
                ...
                with tf.name_scope('p4'):
                    p4 = tf.nn.softmax(b0 + tf.matmul(hidden4_6_drop, w0_6))
    ...
    def prepare_session(self):
        ...
        writer = tf.summary.FileWriter("./log/1130_MNIST_MLP_4layer_epoch", sess.graph)
...
if __name__ == '__main__':
    ...
    session_name = "./session_log/saver_1203_MNIST_MLP_4layer_epoch_5"
