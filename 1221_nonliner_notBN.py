...
class layer:
    ...
    def prepare_model(self):
        ...
        with tf.name_scope('feed_forword'):
            hidden_8 = tf.nn.relu(tf.matmul(x, w8) + b8)
            hidden_7 = tf.nn.relu(tf.matmul(hidden_8, w7) + b7)
            hidden_6 = tf.nn.relu(tf.matmul(hidden_7, w6) + b6)
            hidden_5 = tf.nn.relu(tf.matmul(hidden_6, w5) + b5)
            hidden_4 = tf.nn.relu(tf.matmul(hidden_5, w4) + b4)
            hidden_3 = tf.nn.relu(tf.matmul(hidden_4, w3) + b3)
            hidden_2 = tf.nn.relu(tf.matmul(hidden_3, w2) + b2)
            ...
            """
            hidden_1_8 = tf.nn.relu(hidden_8 + b1_8)
            p0 = tf.nn.softmax(tf.matmul(hidden_1_8, w0_8) + b0_8)

            hidden_1_7 = tf.nn.relu((hidden_8 + hidden_7)/2 + b1_7)
            p1 = tf.nn.softmax(tf.matmul(hidden_1_7, w0_7) + b0_7)

            hidden_1_6 = tf.nn.relu((hidden_8 + hidden_7 + hidden_6)/3 + b1_6)
            p2 = tf.nn.softmax(tf.matmul(hidden_1_6, w0_6) + b0_6)

            hidden_1_5 = tf.nn.relu((hidden_8 + hidden_7 + hidden_6 + hidden_5)/4 + b1_5)
            p3 = tf.nn.softmax(tf.matmul(hidden_1_5, w0_5) + b0_5)

            hidden_1_4 = tf.nn.relu((hidden_8 + hidden_7 + hidden_6 + hidden_5 + hidden_4)/5 + b1_4)
            p4 = tf.nn.softmax(tf.matmul(hidden_1_4, w0_4) + b0_4)

            hidden_1_3 = tf.nn.relu((hidden_8 + hidden_7 + hidden_6 + hidden_5 + hidden_4 + hidden_3)/6 + b1_3)
            p5 = tf.nn.softmax(tf.matmul(hidden_1_3, w0_3) + b0_3)

            hidden_1_2 = tf.nn.relu((hidden_8 + hidden_7 + hidden_6 + hidden_5 + hidden_4 + hidden_3 + hidden_2)/7 + b1_2)
            p6 = tf.nn.softmax(tf.matmul(hidden_1_2, w0_2) + b0_2)
        ...
        with tf.name_scope('train_step0'):
            ...
            with tf.control_dependencies(update_ops):
                ...
                """
                train_step0 = tf.train.AdamOptimizer().minimize(loss0, var_list=[w0_8, b0_8, b1_8, w8, b8])
                train_step1 = tf.train.AdamOptimizer().minimize(loss1, var_list=[w0_7, b0_7, b1_7, w7, b7])
                train_step2 = tf.train.AdamOptimizer().minimize(loss2, var_list=[w0_6, b0_6, b1_6, w6, b6])
                train_step3 = tf.train.AdamOptimizer().minimize(loss3, var_list=[w0_5, b0_5, b1_5, w5, b5])
                train_step4 = tf.train.AdamOptimizer().minimize(loss4, var_list=[w0_4, b0_4, b1_4, w4, b4])
                train_step5 = tf.train.AdamOptimizer().minimize(loss5, var_list=[w0_3, b0_3, b1_3, w3, b3])
                train_step6 = tf.train.AdamOptimizer().minimize(loss6, var_list=[w0_2, b0_2, b1_2, w2, b2])
                """
    ...
    def prepare_session(self):
        ...
        writer = tf.summary.FileWriter("./log/1221_nonliner_notBN", sess.graph)
...
if __name__ == '__main__':
    ...
    drop_out_rate = 1.0
    session_name = "./session_log/saver_1221_nonliner_notBN_"
    training = True
    testing = False
    ...
    np.save("nonliner_notBN_graph_y", graph_y)
