...
class layer:
    ...
    def prepare_model(self):
        ...
        with tf.name_scope('layer0'):
            ...
            with tf.name_scope('w6_9'):
                w0_4 = tf.Variable(tf.truncated_normal(shape=[num_units6, 10], mean=0.0, stddev=1 / num_units6))

            with tf.name_scope('b0'):
                # b0 = tf.Variable(tf.constant(0.1, shape=[10]))
                b0 = tf.Variable(tf.zeros([10]))
                b0_9 = tf.Variable(tf.zeros([10]))
                b0_8 = tf.Variable(tf.zeros([10]))
                b0_7 = tf.Variable(tf.zeros([10]))
                b0_6 = tf.Variable(tf.zeros([10]))
        ...
        with tf.name_scope('feed_forword'):
            ...
            with tf.name_scope('state2'):
                with tf.name_scope('hidden2_2'):
                    hidden2_8 = tf.nn.relu(tf.matmul(x, w8_2) + b8)
        ...
        with tf.name_scope('optimizer'):
            ...
            with tf.name_scope('train_step2'):
                train_step2 = tf.train.AdamOptimizer().minimize(loss2, var_list=[b0, w0_7, w7, b7, w8_2, b8])
            with tf.name_scope('train_step3'):
                train_step3 = tf.train.AdamOptimizer().minimize(loss3, var_list=[b0, w0_6, w6, b6, w7_3, b7, w8_3, b8])
            with tf.name_scope('train_step4'):
                train_step4 = tf.train.AdamOptimizer().minimize(loss4)
            # Adam
            """
...
if __name__ == '__main__':
    ...
    session_name = "./session_log/saver_1125_MNIST_nonliner_fine_tuning_1000_4"
