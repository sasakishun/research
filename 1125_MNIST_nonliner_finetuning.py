...
class layer:
    ...
    def prepare_model(self):
        ...
        with tf.name_scope('layer0'):
            ...
            with tf.name_scope('w0'):
                w0_4 = tf.Variable(tf.truncated_normal(shape=[num_units1, output_size], mean=0.0, stddev=1 / num_units1))

            with tf.name_scope('w0_9'):
                w0_9 = tf.Variable(tf.truncated_normal(shape=[input_size, output_size], mean=0.0, stddev=1 / input_size))
            with tf.name_scope('w7_9'):
                w0_8 = tf.Variable(tf.truncated_normal(shape=[num_units8, output_size], mean=0.0, stddev=1 / num_units8))
            with tf.name_scope('w7_9'):
                w0_7 = tf.Variable(tf.truncated_normal(shape=[num_units7, output_size], mean=0.0, stddev=1 / num_units7))
            with tf.name_scope('w6_9'):
                w0_6 = tf.Variable(tf.truncated_normal(shape=[num_units6, output_size], mean=0.0, stddev=1 / num_units6))
            with tf.name_scope('b0'):
                ...
                # gate_9 = tf.Variable(tf.constant(0.1, shape=[output_size]))
                # gate_8 = tf.Variable(tf.constant(0.1, shape=[output_size]))
                # gate_7 = tf.Variable(tf.constant(0.1, shape=[output_size]))
                # gate_6 = tf.Variable(tf.constant(0.1, shape=[output_size]))
        ...
        with tf.name_scope('optimizer'):
            ...
            with tf.name_scope('train_step2'):
                train_step2 = tf.train.AdamOptimizer().minimize(loss2, var_list=[b0, w0_7, w7, b7, w8, b8])
            with tf.name_scope('train_step3'):
                train_step3 = tf.train.AdamOptimizer().minimize(loss3, var_list=[b0, w0_6, w6, b6, w7, b7, w8, b8])
            with tf.name_scope('train_step4'):
                train_step4 = tf.train.AdamOptimizer().minimize(loss4)
            # Adam
            """
