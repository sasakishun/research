...
class layer:
    ...
    def prepare_model(self):
        ...
        with tf.name_scope('pooling'):
            ...
            h_pool_7 = tf.nn.max_pool(tf.nn.relu(tf.layers.batch_normalization(
                h_conv7, training=training) + b_conv7)
                                      , ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding='SAME')
            h_pool_flat_7 = tf.reshape(h_pool_7, [-1, int((image_size / pooling_down) * num_units0)])

            h_pool_6 = tf.nn.max_pool(tf.nn.relu(tf.layers.batch_normalization(
                h_conv6, training=training) + b_conv6)
                                      , ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding='SAME')
            h_pool_flat_6 = tf.reshape(h_pool_6, [-1, int((image_size / pooling_down) * num_units0)])

            h_pool_5 = tf.nn.max_pool(tf.nn.relu(tf.layers.batch_normalization(
                h_conv5, training=training) + b_conv5), ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding='SAME')
            h_pool_flat_5 = tf.reshape(h_pool_5, [-1, int((image_size / pooling_down) * num_units0)])

            h_pool_4 = tf.nn.max_pool(tf.nn.relu(tf.layers.batch_normalization(
                h_conv4, training=training) + b_conv4), ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding='SAME')
            h_pool_flat_4 = tf.reshape(h_pool_4, [-1, int((image_size / pooling_down) * num_units0)])

            h_pool_3 = tf.nn.max_pool(tf.nn.relu(tf.layers.batch_normalization(
                h_conv3, training=training) + b_conv3), ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding='SAME')
            h_pool_flat_3 = tf.reshape(h_pool_3, [-1, int((image_size / pooling_down) * num_units0)])

            h_pool_2 = tf.nn.max_pool(tf.nn.relu(tf.layers.batch_normalization(
                h_conv2, training=training) + b_conv2), ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding='SAME')
            h_pool_flat_2 = tf.reshape(h_pool_2, [-1, int((image_size / pooling_down) * num_units0)])

            h_pool_1 = tf.nn.max_pool(tf.nn.relu(tf.layers.batch_normalization(
                h_conv1, training=training) + b_conv1), ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding='SAME')
            h_pool_flat_1 = tf.reshape(h_pool_1, [-1, int((image_size / pooling_down) * num_units0)])

            h_pool_0 = tf.nn.max_pool(tf.nn.relu(tf.layers.batch_normalization(
                h_conv0, training=training) + b_conv0), ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding='SAME')
        ...
        with tf.name_scope('optimizer'):
            ...
            with tf.name_scope('train_step0'):
                ...
                train_step5 = tf.train.AdamOptimizer().minimize(loss5, var_list=[W_conv3, b_conv3, w1, b1, w0, b0])
                train_step6 = tf.train.AdamOptimizer().minimize(loss6, var_list=[W_conv2, b_conv2, w1, b1, w0, b0])
                train_step7 = tf.train.AdamOptimizer().minimize(loss7, var_list=[W_conv1, b_conv1, w1, b1, w0, b0])
                train_step8 = tf.train.AdamOptimizer().minimize(loss8, var_list=[W_conv0, b_conv0, w1, b1, w0, b0])
            """
            with tf.name_scope('train_step_for_nonliner'):
    ...
    def prepare_session(self):
        ...
        writer = tf.summary.FileWriter("./log/1212_MLP_CNN_batch_normalized", sess.graph)
...
if __name__ == '__main__':
    ...
    session_name = "./session_log/saver_1212_MLP_CNN_batch_normalized"
    training = True
    testing = False
    ...
    for j in range(start, layer_num + 1):
        ...
        for i in range(loop_len):
            ...
            while len(each_epoch) >= batchsize:
                ...
                nn.sess.run(nn.train_step[j], feed_dict={nn.x: batch_xs, nn.t: batch_ts, nn.keep_prob: drop_out_rate, nn.training: training})
