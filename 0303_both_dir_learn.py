...
class layer:
    ...
    def prepare_model(self):
        ...
        with tf.name_scope('back_forword'):
            hidden_5b = tf.nn.relu(tf.layers.batch_normalization(
                tf.matmul(t-b0, tf.transpose(w0_5, perm=None, name='transpose')) + b5, training=training))
            hidden_6b = tf.nn.relu(tf.layers.batch_normalization(
                tf.matmul(hidden_5b, tf.transpose(w5, perm=None, name='transpose')) + b6, training=training))
            hidden_7b = tf.nn.relu(tf.layers.batch_normalization(
                tf.matmul(hidden_6b, tf.transpose(w6, perm=None, name='transpose')) + b7, training=training))
            hidden_8b = tf.nn.relu(tf.layers.batch_normalization(
                tf.matmul(hidden_7b, tf.transpose(w7, perm=None, name='transpose')) + b8, training=training))

            p0b = tf.nn.relu(tf.matmul(hidden_8b, tf.transpose(w8, perm=None, name='transpose')))
            p1b = tf.nn.relu(tf.matmul(hidden_8b, tf.transpose(w8, perm=None, name='transpose')))
            p2b = tf.nn.relu(tf.matmul(hidden_8b, tf.transpose(w8, perm=None, name='transpose')))
            p3b = tf.nn.relu(tf.matmul(hidden_8b, tf.transpose(w8, perm=None, name='transpose')))
            p4b = tf.nn.relu(tf.matmul(hidden_8b, tf.transpose(w8, perm=None, name='transpose')))
            p5b = tf.nn.relu(tf.matmul(hidden_8b, tf.transpose(w8, perm=None, name='transpose')))
            p6b = tf.nn.relu(tf.matmul(hidden_8b, tf.transpose(w8, perm=None, name='transpose')))
        ...
        with tf.name_scope('optimizer'):
            ...
            loss0b = -tf.reduce_sum(x * tf.log(tf.clip_by_value(p0b, 1e-10, 1.0))) / input_batch_size
            loss1b = -tf.reduce_sum(x * tf.log(tf.clip_by_value(p1b, 1e-10, 1.0))) / input_batch_size
            loss2b = -tf.reduce_sum(x * tf.log(tf.clip_by_value(p2b, 1e-10, 1.0))) / input_batch_size
            loss3b = -tf.reduce_sum(x * tf.log(tf.clip_by_value(p3b, 1e-10, 1.0))) / input_batch_size
            loss4b = -tf.reduce_sum(x * tf.log(tf.clip_by_value(p4b, 1e-10, 1.0))) / input_batch_size
            loss5b = -tf.reduce_sum(x * tf.log(tf.clip_by_value(p5b, 1e-10, 1.0))) / input_batch_size
            loss6b = -tf.reduce_sum(x * tf.log(tf.clip_by_value(p6b, 1e-10, 1.0))) / input_batch_size
            lossb = [loss0b, loss1b, loss2b, loss3b, loss4b, loss5b, loss6b]
        ...
        with tf.name_scope('train_step0'):
            ...
            with tf.control_dependencies(update_ops):
                ...
                train_step0_upper = tf.train.AdamOptimizer().minimize(loss0b)
                train_step1_upper = tf.train.AdamOptimizer().minimize(loss1b)
                train_step2_upper = tf.train.AdamOptimizer().minimize(loss2b)
                train_step3_upper = tf.train.AdamOptimizer().minimize(loss3b)
                train_step4_upper = tf.train.AdamOptimizer().minimize(loss4b)
                train_step5_upper = tf.train.AdamOptimizer().minimize(loss5b)
                train_step6_upper = tf.train.AdamOptimizer().minimize(loss6b)
...
if __name__ == '__main__':
    ...
    if args[1] == "cifar":
        ...
        print("Error")
        print("./0115_... dataset train_size layer_num")
        exit()
    ...
    """
    result is written in 0206_log_MLP.txt
    """
    f = open("0303_both_dir_learn.txt", 'a')
    ...
    ax = plt.subplot()
    ax.plot(graph_x, graph_y, linewidth=1.0, marker=".", markersize=10, label="MLP")
    ax.grid(which="both")
    ax.set_xlabel("epoch")
    ax.set_ylabel("accuracy")
    plt.title(str(train_size) + dataset_name + ":" + path.splitext(path.basename(__file__))[0])
    ax.set_xlim([0, loop_count - 1 + early_stopping_num])

    np.save("MLP_y", graph_y)
    if dataset_flag == 0:
        np.save("MLP_mnist", graph_y)
    if dataset_flag == 1:
        np.save("MLP_fashion", graph_y)
    if dataset_flag == 2:
        np.save("MLP_cifar10", graph_y)
    if dataset_flag == 3:
        np.save("MLP_cifar100", graph_y)
