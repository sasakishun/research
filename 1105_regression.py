...
class layer:
    ...
    def prepare_model(self):
        ...
        with tf.name_scope('layer0_output_fully_connected'):
            ...
            with tf.name_scope('w7_9'):
                w0_8 = tf.Variable(
                    tf.truncated_normal(shape=[num_units8, output_size], mean=0.0, stddev=1 / input_size))
            with tf.name_scope('w7_9'):
                w0_7 = tf.Variable(
                    tf.truncated_normal(shape=[num_units7, output_size], mean=0.0, stddev=1 / input_size))
            with tf.name_scope('w6_9'):
                w0_6 = tf.Variable(
                    tf.truncated_normal(shape=[num_units6, output_size], mean=0.0, stddev=1 / input_size))
            with tf.name_scope('w5_9'):
                w0_5 = tf.Variable(
                    tf.truncated_normal(shape=[num_units5, output_size], mean=0.0, stddev=1 / input_size))
            with tf.name_scope('w4_9'):
                w0_4 = tf.Variable(
                    tf.truncated_normal(shape=[num_units4, output_size], mean=0.0, stddev=1 / input_size))
            with tf.name_scope('w3_9'):
                w0_3 = tf.Variable(
                    tf.truncated_normal(shape=[num_units3, output_size], mean=0.0, stddev=1 / input_size))
            with tf.name_scope('w2_9'):
                w0_2 = tf.Variable(
                    tf.truncated_normal(shape=[num_units2, output_size], mean=0.0, stddev=1 / input_size))
    ...
    def prepare_session(self):
        ...
        writer = tf.summary.FileWriter("./log/1025_sin_regression", sess.graph)
...
class mini_layer:
    ...
    def prepare_model(self):
        with tf.name_scope('input_layer'):
            with tf.name_scope('x'):
                ...
                b3 = tf.Variable(tf.constant(0.1, shape=[num_units3]))
                hidden3 = tf.nn.relu(tf.matmul(x, w3) + b3)
        ...
        with tf.name_scope('layer1_fully_connected'):
            ...
            with tf.name_scope('hidden1'):
                hidden1 = tf.nn.sigmoid(tf.matmul(hidden2, w1) + b1)

        with tf.name_scope('layer0_output_fully_connected'):
            with tf.name_scope('w0'):
                w0 = tf.Variable(tf.truncated_normal(shape=[num_units1, output_size], mean=0.0, stddev=1 / num_units1))
            with tf.name_scope('b0'):
                b0 = tf.Variable(tf.constant(0.1, shape=[output_size]))
            with tf.name_scope('p'):
                p = tf.nn.sigmoid(tf.matmul(hidden1, w0) + b0)
                # p = tf.matmul(hidden1, w0) + b0
        ...
        with tf.name_scope('optimizer'):
            with tf.name_scope('loss'):
                ...
                loss = tf.reduce_sum(tf.square(tf.clip_by_value(p, 1e-10, 1.0) - t))
        ...
        tf.summary.scalar("t", tf.reduce_sum(t))
        tf.summary.scalar("p", tf.reduce_sum(p))
    ...
    def prepare_session(self):
        ...
        sess.run(tf.initialize_all_variables())
        summary = tf.summary.merge_all()
        writer = tf.summary.FileWriter("./log/1105_regression", sess.graph)
        # ここでログファイルを保存するディレクトリとファイル名を決定する
...
if __name__ == '__main__':
    ...
    under_range = 0.0
    max_range = 10.0
    terms = input_size  # 1+x
    train_x = np.array(
        [[random.uniform(under_range, max_range) ** j
          for j in range(terms)] for i in range(sample_size)])

    train_t = np.array([teacher(train_x[i, 1]) for i in range(sample_size)]).reshape([sample_size, 1])
    ...
    for j in range(2):
        for i in range(loop_len):
            ...
            train_t = np.array([teacher(train_x[i, 1]) for i in range(sample_size)]).reshape([sample_size, 1])
            """
            nn.sess.run(nn.train_step, feed_dict={nn.x: train_x, nn.t: train_t})
            if i % 100 == 0:
                """
                # test_data
                ...
                print('Step: %d, Loss: %f, Accuracy: %f' % (i + j * loop_len, loss_val, acc_val))
                nn.sess.run(nn.assign_op_total_accuracy, feed_dict={nn.input_placeholder_total_accuracy: acc_val})
                """
                print(nn.sess.run(nn.p, feed_dict={nn.x: (np.array([random.uniform(under_range, max_range) ** j
                                                                    for j in range(terms)])).reshape(1, -1)
                                                   }))
                # train_data
                summary, loss_val, acc_val = nn.sess.run(
                    [nn.summary, nn.loss, nn.accuracy],
                    feed_dict={nn.x: train_x, nn.t: train_t})
        ...
        subplot.set_xlim(under_range, max_range)
        subplot.scatter(train_x[::, 1:2], train_t, s=1, c="red")
        linex = np.linspace(under_range, max_range, 100)
        input_x = np.array([[linex[i] ** j
                             for j in range(terms)] for i in range(100)])

        liney = nn.sess.run(nn.p, feed_dict={nn.x: input_x})
        subplot.plot(linex, liney, markersize=0.1)
        plt.show()
        loop_len *= 4
