...
class layer:
    ...
    def prepare_model(self):
        ...
        with tf.name_scope('feed_forword'):
            with tf.name_scope('state0'):
                with tf.name_scope('p0'):
                    p0 = tf.nn.softmax(tf.matmul(x, w0_9) * gate_9
                                       + b0)
        ...
        with tf.name_scope('optimizer'):
            ...
            with tf.name_scope('train_step0'):
                train_step0 = tf.train.AdamOptimizer(learning_rate).minimize(loss0)
            ...
            with tf.name_scope('train_step3'):
                train_step3 = tf.train.AdamOptimizer(learning_rate).minimize(loss3)
            with tf.name_scope('train_step4'):
                train_step4 = tf.train.AdamOptimizer(learning_rate).minimize(loss4)
            with tf.name_scope('train_step5'):
                train_step5 = tf.train.AdamOptimizer(learning_rate).minimize(loss5)
            with tf.name_scope('train_step6'):
                train_step6 = tf.train.AdamOptimizer(learning_rate).minimize(loss6)
            with tf.name_scope('train_step7'):
                train_step7 = tf.train.AdamOptimizer(learning_rate).minimize(loss7)
            with tf.name_scope('train_step8'):
                train_step8 = tf.train.AdamOptimizer(learning_rate).minimize(loss8)
            # Adadelta
            """
            with tf.name_scope('train_step0'):
    ...
    def prepare_session(self):
        ...
        writer = tf.summary.FileWriter("./log/1116_MNIST_nonliner_only_proposal", sess.graph)
