...
class Main_test():
    ...
    def test(self, loadflag=True):
        ...
        for i in range(10):
            ...
            visualize_network(
                ...
                comment="just before pruning_stage:{}\n".format(i)
                        + "train:{} test:{}".format(_mlp.evaluate(X_train, y_train)[1],
        ...
        for target_layer in range(1, len(_mlp.get_weights()) // 2 - 1):
        ...
        _mlp = myMLP([input_size]+[np.shape(i)[0] for i in masked_mlp_model.get_weights() if i.ndim == 1])
