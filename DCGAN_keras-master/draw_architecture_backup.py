    ...
    def __intialise_neurons(self, number_of_neurons, is_image=False):
        ...
        for iteration in range(number_of_neurons):
            if is_image:
                ...
                neuron = Neuron(x,
                                self.y - horizontal_distance_between_neurons * np.sqrt(number_of_neurons)
                                * (iteration // np.sqrt(number_of_neurons))
                                - horizontal_distance_between_neurons)  # 40)
                neurons.append(neuron)
                if MNIST:
                    x += horizontal_distance_between_neurons * (784 / np.sqrt(number_of_neurons))
                else:
                ...
                if MNIST:
                    x += horizontal_distance_between_neurons * (784 / number_of_neurons)
                else:
...
def mydraw(_weights, acc=None, comment="", non_active_neurons=None, node_colors=None, dir=None, annotation=None,
    ...
    if nodes[0] == 64:
        ...
        number_of_neurons_in_widest_layer = np.sqrt(784)
    else:
    ...
    for i in range(len(nodes) - 1):
        network.add_layer(nodes[i], weights[i].T, non_active_neurons[i] if non_active_neurons is not None else None,
                          ...
                          is_image=True if (i == 0 and (nodes[0] == 64 or 784)) else False)
