...
class layer:
    ...
    def prepare_model(self):
        ...
        with tf.name_scope('node_size'):
            ...
            with tf.name_scope('num_units6'):
                ...
                num_units0 = hidden_unit  # output_size
        """
        with tf.name_scope('network_architecture'):
            with tf.name_scope('layer8'):
                W_conv8 = tf.Variable(
                    tf.random_uniform(shape=[3, 3, input_channel, num_units8],
                                      minval=-np.sqrt(6.0 / (3 * 3 * input_channel + input_width**2 * num_units8)),
                                      maxval=np.sqrt(6.0 / (3 * 3 * input_channel + input_width**2 * num_units8)),
                                      dtype=tf.float32))
            ...
            with tf.name_scope('layer7'):
                W_conv7 = tf.Variable(
                    tf.random_uniform(shape=[3, 3, num_units8, num_units7],
                                      minval=-np.sqrt(6.0 / (3 * 3 * num_units8 + input_width**2 * num_units7)),
                                      maxval=np.sqrt(6.0 / (3 * 3 * num_units8 + input_width**2 * num_units7)),
                                      dtype=tf.float32))
            ...
            with tf.name_scope('layer6'):
                W_conv6 = tf.Variable(
                    tf.random_uniform(shape=[3, 3, num_units7, num_units6],
                                      minval=-np.sqrt(6.0 / (3 * 3 * num_units7 + input_width**2 * num_units6)),
                                      maxval=np.sqrt(6.0 / (3 * 3 * num_units7 + input_width**2 * num_units6)),
                                      dtype=tf.float32))
            ...
            with tf.name_scope('layer5'):
                W_conv5 = tf.Variable(
                    tf.random_uniform(shape=[3, 3, num_units6, num_units5],
                                      minval=-np.sqrt(6.0 / (3 * 3 * num_units6 + input_width**2 * num_units5)),
                                      maxval=np.sqrt(6.0 / (3 * 3 * num_units6 + input_width**2 * num_units5)),
                                      dtype=tf.float32))
            ...
            with tf.name_scope('layer4'):
                W_conv4 = tf.Variable(
                    tf.random_uniform(shape=[3, 3, num_units5, num_units4],
                                      minval=-np.sqrt(6.0 / (3 * 3 * num_units5 + input_width**2 * num_units4)),
                                      maxval=np.sqrt(6.0 / (3 * 3 * num_units5 + input_width**2 * num_units4)),
                                      dtype=tf.float32))
            ...
            with tf.name_scope('layer3'):
                W_conv3 = tf.Variable(
                    tf.random_uniform(shape=[3, 3, num_units4, num_units3],
                                      minval=-np.sqrt(6.0 / (3 * 3 * num_units4 + input_width**2 * num_units3)),
                                      maxval=np.sqrt(6.0 / (3 * 3 * num_units4 + input_width**2 * num_units3)),
                                      dtype=tf.float32))
            ...
            with tf.name_scope('layer2'):
                W_conv2 = tf.Variable(
                    tf.random_uniform(shape=[3, 3, num_units3, num_units2],
                                      minval=-np.sqrt(6.0 / (3 * 3 * num_units3 + input_width**2 * num_units2)),
                                      maxval=np.sqrt(6.0 / (3 * 3 * num_units3 + input_width**2 * num_units2)),
                                      dtype=tf.float32))
            ...
            with tf.name_scope('layer1'):
                W_conv1 = tf.Variable(
                    tf.random_uniform(shape=[3, 3, num_units2, num_units1],
                                      minval=-np.sqrt(6.0 / (3 * 3 * num_units2 + input_width**2 * num_units1)),
                                      maxval=np.sqrt(6.0 / (3 * 3 * num_units2 + input_width**2 * num_units1)),
                                      dtype=tf.float32))
            ...
            with tf.name_scope('layer0'):
                W_conv0 = tf.Variable(
                    tf.random_uniform(shape=[3, 3, num_units1, num_units0],
                                      minval=-np.sqrt(6.0 / (3 * 3 * num_units1 + input_width**2 * num_units0)),
                                      maxval=np.sqrt(6.0 / (3 * 3 * num_units1 + input_width**2 * num_units0)),
                                      dtype=tf.float32))
            ...
            with tf.name_scope('full_connected_layer'):
                w1 = tf.Variable(
                    tf.random_uniform(shape=[int((image_size / pooling_down) * num_units0), 1000],
                                      minval=-np.sqrt(6.0 / ((image_size / pooling_down) * num_units0 + 1000)),
                                      maxval=np.sqrt(6.0 / ((image_size / pooling_down) * num_units0 + 1000)),
                                      dtype=tf.float32))
        ...
        with tf.name_scope('network_architecture'):
            with tf.name_scope('layer8'):
                W_conv8 = tf.Variable(
                    tf.random_uniform(shape=[3, 3, input_channel, num_units8],
                                      minval=-np.sqrt(6.0 / (3 * 3 * input_channel + 3 * 3 * num_units8)),
                                      maxval=np.sqrt(6.0 / (3 * 3 * input_channel + 3 * 3 * num_units8)),
                                      dtype=tf.float32))
                # W_conv8 = tf.Variable(tf.truncated_normal([3, 3, input_channel, num_units8], stddev=0.01))
                b_conv8 = tf.Variable(tf.constant(0.1, shape=[num_units8]))
            with tf.name_scope('layer7'):
                W_conv7 = tf.Variable(
                    tf.random_uniform(shape=[3, 3, num_units8, num_units7],
                                      minval=-np.sqrt(6.0 / (3 * 3 * num_units8 + 3 * 3 * num_units7)),
                                      maxval=np.sqrt(6.0 / (3 * 3 * num_units8 + 3 * 3 * num_units7)),
                                      dtype=tf.float32))
                # W_conv7 = tf.Variable(tf.truncated_normal([3, 3, num_units8, num_units7], stddev=0.01))
                b_conv7 = tf.Variable(tf.constant(0.1, shape=[num_units7]))
            with tf.name_scope('layer6'):
                W_conv6 = tf.Variable(
                    tf.random_uniform(shape=[3, 3, num_units7, num_units6],
                                      minval=-np.sqrt(6.0 / (3 * 3 * num_units7 + 3 * 3 * num_units6)),
                                      maxval=np.sqrt(6.0 / (3 * 3 * num_units7 + 3 * 3 * num_units6)),
                                      dtype=tf.float32))
                # W_conv6 = tf.Variable(tf.truncated_normal([3, 3, num_units7, num_units6], stddev=0.01))
                b_conv6 = tf.Variable(tf.constant(0.1, shape=[num_units6]))
            with tf.name_scope('layer5'):
                W_conv5 = tf.Variable(
                    tf.random_uniform(shape=[3, 3, num_units6, num_units5],
                                      minval=-np.sqrt(6.0 / (3 * 3 * num_units6 + 3 * 3 * num_units5)),
                                      maxval=np.sqrt(6.0 / (3 * 3 * num_units6 + 3 * 3 * num_units5)),
                                      dtype=tf.float32))
                # W_conv5 = tf.Variable(tf.truncated_normal([3, 3, num_units6, num_units5], stddev=0.01))
                b_conv5 = tf.Variable(tf.constant(0.1, shape=[num_units5]))
            with tf.name_scope('layer4'):
                W_conv4 = tf.Variable(
                    tf.random_uniform(shape=[3, 3, num_units5, num_units4],
                                      minval=-np.sqrt(6.0 / (3 * 3 * num_units5 + 3 * 3 * num_units4)),
                                      maxval=np.sqrt(6.0 / (3 * 3 * num_units5 + 3 * 3 * num_units4)),
                                      dtype=tf.float32))
                # W_conv4 = tf.Variable(tf.truncated_normal([3, 3, num_units5, num_units4], stddev=0.01))
                b_conv4 = tf.Variable(tf.constant(0.1, shape=[num_units4]))
            with tf.name_scope('layer3'):
                W_conv3 = tf.Variable(
                    tf.random_uniform(shape=[3, 3, num_units4, num_units3],
                                      minval=-np.sqrt(6.0 / (3 * 3 * num_units4 + 3 * 3 * num_units3)),
                                      maxval=np.sqrt(6.0 / (3 * 3 * num_units4 + 3 * 3 * num_units3)),
                                      dtype=tf.float32))
                # W_conv3 = tf.Variable(tf.truncated_normal([3, 3, num_units4, num_units3], stddev=0.01))
                b_conv3 = tf.Variable(tf.constant(0.1, shape=[num_units3]))
            with tf.name_scope('layer2'):
                W_conv2 = tf.Variable(
                    tf.random_uniform(shape=[3, 3, num_units3, num_units2],
                                      minval=-np.sqrt(6.0 / (3 * 3 * num_units3 + 3 * 3 * num_units2)),
                                      maxval=np.sqrt(6.0 / (3 * 3 * num_units3 + 3 * 3 * num_units2)),
                                      dtype=tf.float32))
                # W_conv2 = tf.Variable(tf.truncated_normal([3, 3, num_units3, num_units2], stddev=0.01))
                b_conv2 = tf.Variable(tf.constant(0.1, shape=[num_units2]))
            with tf.name_scope('layer1'):
                W_conv1 = tf.Variable(
                    tf.random_uniform(shape=[3, 3, num_units2, num_units1],
                                      minval=-np.sqrt(6.0 / (3 * 3 * num_units2 + 3 * 3 * num_units1)),
                                      maxval=np.sqrt(6.0 / (3 * 3 * num_units2 + 3 * 3 * num_units1)),
                                      dtype=tf.float32))
                # W_conv1 = tf.Variable(tf.truncated_normal([3, 3, num_units2, num_units1], stddev=0.01))
                b_conv1 = tf.Variable(tf.constant(0.1, shape=[num_units1]))
            with tf.name_scope('layer0'):
                W_conv0 = tf.Variable(
                    tf.random_uniform(shape=[3, 3, num_units1, num_units0],
                                      minval=-np.sqrt(6.0 / (3 * 3 * num_units1 + 3 * 3 * num_units0)),
                                      maxval=np.sqrt(6.0 / (3 * 3 * num_units1 + 3 * 3 * num_units0)),
                                      dtype=tf.float32))
                # W_conv0 = tf.Variable(tf.truncated_normal([3, 3, num_units1, num_units0], stddev=0.01))
                b_conv0 = tf.Variable(tf.constant(0.1, shape=[num_units0]))
            with tf.name_scope('full_connected_layer'):
        ...
        with tf.name_scope('network_architecture'):
            with tf.name_scope('layer8'):
                W_conv8 = tf.Variable(
                    tf.random_uniform(shape=[3, 3, input_channel, num_units8],
                                      minval=-np.sqrt(6.0 / (3 * 3 * input_channel + num_units8*input_size)),
                                      maxval=np.sqrt(6.0 / (3 * 3 * input_channel + num_units8*input_size)),
                                      dtype=tf.float32))
                # W_conv8 = tf.Variable(tf.truncated_normal([3, 3, input_channel, num_units8], stddev=0.01))
                b_conv8 = tf.Variable(tf.constant(0.1, shape=[num_units8]))
            with tf.name_scope('layer7'):
                W_conv7 = tf.Variable(
                    tf.random_uniform(shape=[3, 3, num_units8, num_units7],
                                      minval=-np.sqrt(6.0 / (3 * 3 * num_units8 + num_units7*input_size)),
                                      maxval=np.sqrt(6.0 / (3 * 3 * num_units8 + num_units7*input_size)),
                                      dtype=tf.float32))
                # W_conv7 = tf.Variable(tf.truncated_normal([3, 3, num_units8, num_units7], stddev=0.01))
                b_conv7 = tf.Variable(tf.constant(0.1, shape=[num_units7]))
            with tf.name_scope('layer6'):
                W_conv6 = tf.Variable(
                    tf.random_uniform(shape=[3, 3, num_units7, num_units6],
                                      minval=-np.sqrt(6.0 / (3 * 3 * num_units7 + num_units6*input_size)),
                                      maxval=np.sqrt(6.0 / (3 * 3 * num_units7 + num_units6*input_size)),
                                      dtype=tf.float32))
                # W_conv6 = tf.Variable(tf.truncated_normal([3, 3, num_units7, num_units6], stddev=0.01))
                b_conv6 = tf.Variable(tf.constant(0.1, shape=[num_units6]))
            with tf.name_scope('layer5'):
                W_conv5 = tf.Variable(
                    tf.random_uniform(shape=[3, 3, num_units6, num_units5],
                                      minval=-np.sqrt(6.0 / (3 * 3 * num_units6 + num_units5*input_size)),
                                      maxval=np.sqrt(6.0 / (3 * 3 * num_units6 + num_units5*input_size)),
                                      dtype=tf.float32))
                # W_conv5 = tf.Variable(tf.truncated_normal([3, 3, num_units6, num_units5], stddev=0.01))
                b_conv5 = tf.Variable(tf.constant(0.1, shape=[num_units5]))
            with tf.name_scope('layer4'):
                W_conv4 = tf.Variable(
                    tf.random_uniform(shape=[3, 3, num_units5, num_units4],
                                      minval=-np.sqrt(6.0 / (3 * 3 * num_units5 + num_units4*input_size)),
                                      maxval=np.sqrt(6.0 / (3 * 3 * num_units5 + num_units4*input_size)),
                                      dtype=tf.float32))
                # W_conv4 = tf.Variable(tf.truncated_normal([3, 3, num_units5, num_units4], stddev=0.01))
                b_conv4 = tf.Variable(tf.constant(0.1, shape=[num_units4]))
            with tf.name_scope('layer3'):
                W_conv3 = tf.Variable(
                    tf.random_uniform(shape=[3, 3, num_units4, num_units3],
                                      minval=-np.sqrt(6.0 / (3 * 3 * num_units4 + num_units3*input_size)),
                                      maxval=np.sqrt(6.0 / (3 * 3 * num_units4 + num_units3*input_size)),
                                      dtype=tf.float32))
                # W_conv3 = tf.Variable(tf.truncated_normal([3, 3, num_units4, num_units3], stddev=0.01))
                b_conv3 = tf.Variable(tf.constant(0.1, shape=[num_units3]))
            with tf.name_scope('layer2'):
                W_conv2 = tf.Variable(
                    tf.random_uniform(shape=[3, 3, num_units3, num_units2],
                                      minval=-np.sqrt(6.0 / (3 * 3 * num_units3 + num_units2*input_size)),
                                      maxval=np.sqrt(6.0 / (3 * 3 * num_units3 + num_units2*input_size)),
                                      dtype=tf.float32))
                # W_conv2 = tf.Variable(tf.truncated_normal([3, 3, num_units3, num_units2], stddev=0.01))
                b_conv2 = tf.Variable(tf.constant(0.1, shape=[num_units2]))
            with tf.name_scope('layer1'):
                W_conv1 = tf.Variable(
                    tf.random_uniform(shape=[3, 3, num_units2, num_units1],
                                      minval=-np.sqrt(6.0 / (3 * 3 * num_units2 + num_units1*input_size)),
                                      maxval=np.sqrt(6.0 / (3 * 3 * num_units2 + num_units1*input_size)),
                                      dtype=tf.float32))
                # W_conv1 = tf.Variable(tf.truncated_normal([3, 3, num_units2, num_units1], stddev=0.01))
                b_conv1 = tf.Variable(tf.constant(0.1, shape=[num_units1]))
            with tf.name_scope('layer0'):
                W_conv0 = tf.Variable(
                    tf.random_uniform(shape=[3, 3, num_units1, num_units0],
                                      minval=-np.sqrt(6.0 / (3 * 3 * num_units1 + num_units0*input_size)),
                                      maxval=np.sqrt(6.0 / (3 * 3 * num_units1 + num_units0*input_size)),
                                      dtype=tf.float32))
                # W_conv0 = tf.Variable(tf.truncated_normal([3, 3, num_units1, num_units0], stddev=0.01))
                b_conv0 = tf.Variable(tf.constant(0.1, shape=[num_units0]))
            with tf.name_scope('full_connected_layer'):
        ...
        with tf.name_scope('convolution'):
            h_conv8 = tf.nn.conv2d(x_image, W_conv8, strides=[1, 1, 1, 1], padding='SAME')
            h_conv8_cutoff = tf.nn.relu(h_conv8 + b_conv8)

            h_conv7 = tf.nn.conv2d(h_conv8_cutoff, W_conv7, strides=[1, 1, 1, 1], padding='SAME')
            h_conv7_cutoff = tf.nn.relu(h_conv7 + b_conv7)

            h_conv6 = tf.nn.conv2d(h_conv7_cutoff, W_conv6, strides=[1, 1, 1, 1], padding='SAME')
            h_conv6_cutoff = tf.nn.relu(h_conv6 + b_conv6)

            h_conv5 = tf.nn.conv2d(h_conv6_cutoff, W_conv5, strides=[1, 1, 1, 1], padding='SAME')
            ...
            h_conv3 = tf.nn.conv2d(h_conv4_cutoff, W_conv3, strides=[1, 1, 1, 1], padding='SAME')
            ...
            h_conv1 = tf.nn.conv2d(h_conv2_cutoff, W_conv1, strides=[1, 1, 1, 1], padding='SAME')
            h_conv1_cutoff = tf.nn.relu(h_conv1 + b_conv1)

            h_conv0 = tf.nn.conv2d(h_conv1_cutoff, W_conv0, strides=[1, 1, 1, 1], padding='SAME')
            h_conv0_cutoff = tf.nn.relu(h_conv0 + b_conv0)
        with tf.name_scope('pooling'):
            h_pool_8 = tf.nn.max_pool(h_conv8_cutoff, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding='SAME')
            h_pool_flat_8 = tf.reshape(h_pool_8, [-1, int((image_size / pooling_down) * num_units0)])
            ...
            hidden_1_7 = tf.nn.relu(tf.matmul(h_pool_flat_7, w1) + b1)
            hidden_1_6 = tf.nn.relu(tf.matmul(h_pool_flat_6, w1) + b1)
            hidden_1_5 = tf.nn.relu(tf.matmul(h_pool_flat_5, w1) + b1)
            hidden_1_4 = tf.nn.relu(tf.matmul(h_pool_flat_4, w1) + b1)
            hidden_1_3 = tf.nn.relu(tf.matmul(h_pool_flat_3, w1) + b1)
            hidden_1_2 = tf.nn.relu(tf.matmul(h_pool_flat_2, w1) + b1)
            hidden_1_1 = tf.nn.relu(tf.matmul(h_pool_flat_1, w1) + b1)
            hidden_1_0 = tf.nn.relu(tf.matmul(h_pool_flat_0, w1) + b1)
            ...
            with tf.name_scope('p1'):
                p1 = tf.nn.softmax(tf.matmul(hidden_1_7, w0) + b0)
            with tf.name_scope('p2'):
                p2 = tf.nn.softmax(tf.matmul(hidden_1_6, w0) + b0)
            with tf.name_scope('p3'):
                p3 = tf.nn.softmax(tf.matmul(hidden_1_5, w0) + b0)
            with tf.name_scope('p4'):
                p4 = tf.nn.softmax(tf.matmul(hidden_1_4, w0) + b0)
            with tf.name_scope('p5'):
                p5 = tf.nn.softmax(tf.matmul(hidden_1_3, w0) + b0)
            with tf.name_scope('p6'):
                p6 = tf.nn.softmax(tf.matmul(hidden_1_2, w0) + b0)
            with tf.name_scope('p7'):
                p7 = tf.nn.softmax(tf.matmul(hidden_1_1, w0) + b0)
            with tf.name_scope('p8'):
                p8 = tf.nn.softmax(tf.matmul(hidden_1_0, w0) + b0)
        ...
        with tf.name_scope('optimizer'):
            ...
            with tf.name_scope('loss4'):
                loss5 = -tf.reduce_sum(t * tf.log(tf.clip_by_value(p5, 1e-10, 1.0))) / batchsize
            with tf.name_scope('loss4'):
                loss6 = -tf.reduce_sum(t * tf.log(tf.clip_by_value(p6, 1e-10, 1.0))) / batchsize
            with tf.name_scope('loss4'):
                loss7 = -tf.reduce_sum(t * tf.log(tf.clip_by_value(p7, 1e-10, 1.0))) / batchsize
            with tf.name_scope('loss4'):
                loss8 = -tf.reduce_sum(t * tf.log(tf.clip_by_value(p8, 1e-10, 1.0))) / batchsize
            ...
            with tf.name_scope('train_step0'):
                ...
                train_step4 = tf.train.AdamOptimizer().minimize(loss4,
                                                                var_list=[b0, w8, b8, w7, b7, w6, b6, w5, b5, gate_6, w0_5])
                train_step5 = tf.train.AdamOptimizer().minimize(loss5,
                                                                var_list=[b0, w8, b8, w7, b7, w6, b6, w5, b5, w4, b4, gate_5, w0_4])
                train_step6 = tf.train.AdamOptimizer().minimize(loss6,
                                                                var_list=[b0, w8, b8, w7, b7, w6, b6, w5, b5, w4, b4, w3, b3, gate_4, w0_3])
                train_step7 = tf.train.AdamOptimizer().minimize(loss7,
                                                                var_list=[b0, w8, b8, w7, b7, w6, b6, w5, b5, w4, b4, w3, b3, w2, b2, gate_3, w0_2])
                train_step8 = tf.train.AdamOptimizer().minimize(loss8,
                                                                var_list=[b0, w8, b8, w7, b7, w6, b6, w5, b5, w4, b4, w3, b3, w2, b2, w1, b1, gate_2, w0])
                """
                train_step0 = tf.train.AdamOptimizer().minimize(loss0)
                train_step1 = tf.train.AdamOptimizer().minimize(loss1)
                train_step2 = tf.train.AdamOptimizer().minimize(loss2)
            ...
            train_step = [train_step0, train_step1, train_step2, train_step3, train_step4,
                          train_step5, train_step6, train_step7, train_step8]
    ...
    def prepare_session(self):
        ...
        writer = tf.summary.FileWriter("./log/1211_MLP_CNN", sess.graph)
...
if __name__ == '__main__':
    ...
    session_name = "./session_log/saver_1211_MLP_CNN_"
