...
class layer:
    ...
    def prepare_model(self):
        ...
        with tf.name_scope('feed_forword'):
            ...
            with tf.name_scope('state2'):
                ...
                with tf.name_scope('p2'):
                    p2 = tf.nn.softmax(
                        tf.matmul(hidden2_7_drop, w0_7) + b0 + tf.matmul(x, w0_9) + tf.matmul(hidden2_7, w0_8))
            ...
            with tf.name_scope('state3'):
                ...
                with tf.name_scope('p3'):
                    p3 = tf.nn.softmax(
                        tf.matmul(x, w0_9) + b0 + tf.matmul(hidden3_6_drop, w0_6) + tf.matmul(hidden3_7_drop,
                                                                                              w0_7) + tf.matmul(
                            hidden3_8_drop, w0_8))
        ...
        with tf.name_scope('optimizer'):
            ...
            with tf.name_scope('loss2'):
                loss2 = -tf.reduce_sum(t * tf.log(tf.clip_by_value(p2, 1e-10, 1.0)))  # - tf.reduce_sum(tf.log(tf.clip_by_value(w0_9, 1e-10, 1.0)))
            with tf.name_scope('loss3'):
                loss3 = -tf.reduce_sum(t * tf.log(tf.clip_by_value(p3, 1e-10, 1.0)))  # - tf.reduce_sum(tf.log(tf.clip_by_value(w0_8, 1e-10, 1.0)))
            with tf.name_scope('loss4'):
                loss4 = -tf.reduce_sum(t * tf.log(tf.clip_by_value(p4, 1e-10, 1.0)))  # - tf.reduce_sum(tf.log(tf.clip_by_value(w0_7, 1e-10, 1.0)))
            with tf.name_scope('loss5'):
                loss5 = -tf.reduce_sum(t * tf.log(tf.clip_by_value(p5, 1e-10, 1.0)))  # - tf.reduce_sum(tf.log(tf.clip_by_value(w0_6, 1e-10, 1.0)))
            with tf.name_scope('loss6'):
                loss6 = -tf.reduce_sum(t * tf.log(tf.clip_by_value(p6, 1e-10, 1.0)))  # - tf.reduce_sum(tf.log(tf.clip_by_value(w0_5, 1e-10, 1.0)))
            with tf.name_scope('loss7'):
                loss7 = -tf.reduce_sum(t * tf.log(tf.clip_by_value(p7, 1e-10, 1.0)))  # - tf.reduce_sum(tf.log(tf.clip_by_value(w0_4, 1e-10, 1.0)))
            with tf.name_scope('loss8'):
                loss8 = -tf.reduce_sum(t * tf.log(tf.clip_by_value(p8, 1e-10, 1.0)))  # - tf.reduce_sum(tf.log(tf.clip_by_value(w0_3, 1e-10, 1.0)))
            ...
            with tf.name_scope('train_step3'):
                train_step3 = tf.train.AdamOptimizer(0.0001).minimize(loss3, var_list=[w8, b8, w7, b7, w6, b6, w0_6, b0])
            with tf.name_scope('train_step4'):
                train_step4 = tf.train.AdamOptimizer(0.0001).minimize(loss4, var_list=[w8, b8, w7, b7, w6, b6, w5, b5, w0_5, b0])
            with tf.name_scope('train_step5'):
                train_step5 = tf.train.AdamOptimizer(0.0001).minimize(loss5, var_list=[w8, b8, w7, b7, w6, b6, w5, b5, w4, b4, w0_4, b0])
            with tf.name_scope('train_step6'):
                train_step6 = tf.train.AdamOptimizer(0.0001).minimize(loss6, var_list=[w8, b8, w7, b7, w6, b6, w5, b5, w4, b4, w3, b3, w0_3, b0])
            with tf.name_scope('train_step7'):
                train_step7 = tf.train.AdamOptimizer(0.0001).minimize(loss7, var_list=[w8, b8, w7, b7, w6, b6, w5, b5, w4, b4, w3, b3, w2, b2, w0_2, b0])
            with tf.name_scope('train_step8'):
                train_step8 = tf.train.AdamOptimizer(0.0001).minimize(loss8, var_list=[w8, b8, w7, b7, w6, b6, w5, b5, w4, b4, w3, b3, w2, b2, w1, b1, w0, b0])
    ...
    def prepare_session(self):
        ...
        writer = tf.summary.FileWriter("./log/1025_add_layer_in_front_of_top", sess.graph)
...
if __name__ == '__main__':
    ...
    for j in range(len(nn.train_step)):
        ...
        for i in range(loop_len):
            ...
            # nn.sess.run(nn.assign_op0_9, feed_dict={nn.input_placeholder0_9: w0_9})
            if i % 100 == 0:
