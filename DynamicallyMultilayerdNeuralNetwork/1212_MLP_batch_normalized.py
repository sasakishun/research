...
class layer:
    ...
    def prepare_model(self):
        ...
        with tf.name_scope('feed_forword'):
            ...
            with tf.name_scope('p1'):
                hidden_1_7 = tf.nn.relu((tf.layers.batch_normalization(hidden_7, training=training) + b1))
                p1 = tf.nn.softmax(tf.matmul(hidden_1_7, w0) + b0)
            with tf.name_scope('p2'):
                hidden_1_6 = tf.nn.relu((tf.layers.batch_normalization(hidden_6, training=training) + b1))
                p2 = tf.nn.softmax(tf.matmul(hidden_1_6, w0) + b0)
            with tf.name_scope('p3'):
                hidden_1_5 = tf.nn.relu((tf.layers.batch_normalization(hidden_5, training=training) + b1))
                p3 = tf.nn.softmax(tf.matmul(hidden_1_5, w0) + b0)
            with tf.name_scope('p4'):
                hidden_1_4 = tf.nn.relu((tf.layers.batch_normalization(hidden_4, training=training) + b1))
                p4 = tf.nn.softmax(tf.matmul(hidden_1_4, w0) + b0)
            with tf.name_scope('p5'):
                hidden_1_3 = tf.nn.relu((tf.layers.batch_normalization(hidden_3, training=training) + b1))
                p5 = tf.nn.softmax(tf.matmul(hidden_1_3, w0) + b0)
            with tf.name_scope('p6'):
                hidden_1_2 = tf.nn.relu((tf.layers.batch_normalization(hidden_2, training=training) + b1))
                p6 = tf.nn.softmax(tf.matmul(hidden_1_2, w0) + b0)
            """
            with tf.name_scope('p0'):
                p0 = tf.nn.softmax(tf.layers.batch_normalization(tf.matmul(hidden_8, w0), training=training) + b0)
            with tf.name_scope('p1'):
                p1 = tf.nn.softmax(tf.layers.batch_normalization(tf.matmul(hidden_7, w0), training=training) + b0)
            with tf.name_scope('p2'):
                p2 = tf.nn.softmax(tf.layers.batch_normalization(tf.matmul(hidden_6, w0), training=training) + b0)
            with tf.name_scope('p3'):
                p3 = tf.nn.softmax(tf.layers.batch_normalization(tf.matmul(hidden_5, w0), training=training) + b0)
            with tf.name_scope('p4'):
                p4 = tf.nn.softmax(tf.layers.batch_normalization(tf.matmul(hidden_4, w0), training=training) + b0)
            with tf.name_scope('p5'):
                p5 = tf.nn.softmax(tf.layers.batch_normalization(tf.matmul(hidden_3, w0), training=training) + b0)
            with tf.name_scope('p6'):
                p6 = tf.nn.softmax(tf.layers.batch_normalization(tf.matmul(hidden_2, w0), training=training) + b0)
    ...
    def prepare_session(self):
        ...
        writer = tf.summary.FileWriter("./log/1212_MLP_batch_normalized", sess.graph)
...
if __name__ == '__main__':
    ...
    session_name = "./session_log/saver_1212_MLP_batch_normalized_2"
    training = True
    testing = False
