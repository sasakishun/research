...
class layer:
    ...
    def prepare_model(self):
        ...
        with tf.name_scope('feed_forword'):
            ...
            with tf.name_scope('p0'):
                #hidden_1_8 = hidden_8
                hidden_1_8 = tf.nn.relu((tf.layers.batch_normalization(hidden_8 + b1, training=training)))
            ...
            p4 = tf.nn.softmax(tf.layers.batch_normalization(tf.matmul(hidden_1_4, w0), training=training) + b0)
            p5 = tf.nn.softmax(tf.layers.batch_normalization(tf.matmul(hidden_1_3, w0), training=training) + b0)
            p6 = tf.nn.softmax(tf.layers.batch_normalization(tf.matmul(hidden_1_2, w0), training=training) + b0)
            """
            ...
            with tf.name_scope('p0'):
                hidden_1_8 = tf.nn.relu(hidden_8 + b1)
                hidden_1_7 = tf.nn.relu((hidden_8 + hidden_7)/2 + b1)
                hidden_1_6 = tf.nn.relu((hidden_8 + hidden_7 + hidden_6)/3 + b1)
                hidden_1_5 = tf.nn.relu((hidden_8 + hidden_7 + hidden_6 + hidden_5)/4 + b1)
                hidden_1_4 = tf.nn.relu((hidden_8 + hidden_7 + hidden_6 + hidden_5 + hidden_4)/5 + b1)
                hidden_1_3 = tf.nn.relu((hidden_8 + hidden_7 + hidden_6 + hidden_5 + hidden_4 + hidden_3)/6 + b1)
                hidden_1_2 = tf.nn.relu((hidden_8 + hidden_7 + hidden_6 + hidden_5 + hidden_4 + hidden_3 + hidden_2)/7 + b1)
    ...
    def prepare_session(self):
        ...
        writer = tf.summary.FileWriter("./log/1212_nonliner_batch_normalized", sess.graph)
...
if __name__ == '__main__':
    ...
    session_name = "./session_log/saver_1212_nonliner_batch_normalized_"
    training = True
    testing = False
