...
if __name__ == '__main__':
    ...
    under_range = 0.0
    max_range = 10.0
    train_x = np.array([random.uniform(under_range, max_range) for i in range(sample_size)]).reshape([sample_size, 1])
    train_t = np.array([np.sin(train_x[i]) for i in range(sample_size)]).reshape([sample_size, 1])
    ...
    for j in range(2):
        for i in range(loop_len):
            ...
            # print(nn.sess.run(nn.p, feed_dict={nn.x: train_x, nn.keep_prob: 1.0}))
            if i % 100 == 0:
                """
                # test_data
                ...
                """
                print(nn.sess.run(nn.p, feed_dict={nn.x: np.array([0.1]).reshape([1, 1])}))
                # train_data
                summary, loss_val, acc_val = nn.sess.run(
        ...
        subplot.scatter(train_x, train_t, s=1, c="red")
        linex = np.linspace(under_range, max_range, 100)
        liney = nn.sess.run(nn.p, feed_dict={nn.x: linex.reshape([100, 1])})
