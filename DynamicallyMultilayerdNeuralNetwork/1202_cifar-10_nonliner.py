...
class layer:
    ...
    def prepare_session(self):
        ...
        writer = tf.summary.FileWriter("./log/1202_cifar-10_nonliner/gate_placeholder", sess.graph)
...
if __name__ == '__main__':
    ...
    session_name = "./session_log/saver_1202_cifar-10_nonliner_8"
    ...
    for j in range(3):
        ...
        print("------test__end------")
        if j == 0:
            nn.sess.run(nn.gate_9, feed_dict={nn.gate_9: [valid_acc]})
        if j == 1:
            nn.sess.run(nn.gate_8, feed_dict={nn.gate_8: [valid_acc]})
        if j == 2:
            nn.sess.run(nn.gate_7, feed_dict={nn.gate_7: [valid_acc]})
        j += 1
