...
class layer:
    ...
    def prepare_model(self):
        ...
        with tf.name_scope('layer0_shortcut'):
            ...
            with tf.name_scope('w0_5'):
                w0_5 = tf.Variable(tf.random_uniform(shape=[num_units5, output_size],
                                                     minval=-np.sqrt(6.0 / (num_units5 + output_size)),
                                                     maxval=np.sqrt(6.0 / (num_units5 + output_size)),
                                                     dtype=tf.float32))
            with tf.name_scope('w0_4'):
                w0_4 = tf.Variable(tf.random_uniform(shape=[num_units4, output_size],
                                                     minval=-np.sqrt(6.0 / (num_units4 + output_size)),
                                                     maxval=np.sqrt(6.0 / (num_units4 + output_size)),
                                                     dtype=tf.float32))
            with tf.name_scope('w0_3'):
                w0_3 = tf.Variable(tf.random_uniform(shape=[num_units3, output_size],
                                                     minval=-np.sqrt(6.0 / (num_units3 + output_size)),
                                                     maxval=np.sqrt(6.0 / (num_units3 + output_size)),
                                                     dtype=tf.float32))
            with tf.name_scope('w0_2'):
                w0_2 = tf.Variable(tf.random_uniform(shape=[num_units2, output_size],
                                                     minval=-np.sqrt(6.0 / (num_units2 + output_size)),
                                                     maxval=np.sqrt(6.0 / (num_units2 + output_size)),
                                                     dtype=tf.float32))
        ...
        with tf.name_scope('feed_forword'):
            hidden_8 = tf.nn.relu(tf.layers.batch_normalization(tf.matmul(x, w8), training=training) + b8)
            hidden_8_drop = tf.nn.dropout(hidden_8, keep_prob)
            hidden_7 = tf.nn.relu(tf.layers.batch_normalization(tf.matmul(hidden_8, w7), training=training) + b7)
            hidden_7_drop = tf.nn.dropout(hidden_7, keep_prob)
            hidden_6 = tf.nn.relu(tf.layers.batch_normalization(tf.matmul(hidden_7, w6), training=training) + b6)
            hidden_6_drop = tf.nn.dropout(hidden_6, keep_prob)
            hidden_5 = tf.nn.relu(tf.layers.batch_normalization(tf.matmul(hidden_6, w5), training=training) + b5)
            hidden_5_drop = tf.nn.dropout(hidden_5, keep_prob)
            hidden_4 = tf.nn.relu(tf.layers.batch_normalization(tf.matmul(hidden_5, w4), training=training) + b4)
            hidden_4_drop = tf.nn.dropout(hidden_4, keep_prob)
            hidden_3 = tf.nn.relu(tf.layers.batch_normalization(tf.matmul(hidden_4, w3), training=training) + b3)
            hidden_3_drop = tf.nn.dropout(hidden_3, keep_prob)
            hidden_2 = tf.nn.relu(tf.layers.batch_normalization(tf.matmul(hidden_3, w2), training=training) + b2)
            hidden_2_drop = tf.nn.dropout(hidden_2, keep_prob)

            with tf.name_scope('p0'):
                hidden_1_8 = tf.nn.relu((tf.layers.batch_normalization(hidden_8_drop,
                                                                       training=training) + b1))
                hidden_1_8_drop = tf.nn.dropout(hidden_1_8, keep_prob)
                p0 = tf.nn.softmax(tf.layers.batch_normalization(tf.matmul(hidden_1_8_drop, w0)) + b0)
            with tf.name_scope('p1'):
                hidden_1_7 = tf.nn.relu((tf.layers.batch_normalization(hidden_8_drop + hidden_7_drop,
                                                                       training=training)/2 + b1))
                hidden_1_7_drop = tf.nn.dropout(hidden_1_7, keep_prob)
                p1 = tf.nn.softmax(tf.layers.batch_normalization(tf.matmul(hidden_1_7_drop, w0)) + b0)
            with tf.name_scope('p2'):
                hidden_1_6 = tf.nn.relu((tf.layers.batch_normalization(hidden_8_drop + hidden_7_drop + hidden_6_drop,
                                                                       training=training)/3 + b1))
                hidden_1_6_drop = tf.nn.dropout(hidden_1_6, keep_prob)
                p2 = tf.nn.softmax(tf.layers.batch_normalization(tf.matmul(hidden_1_6_drop, w0)) + b0)
            with tf.name_scope('p3'):
                hidden_1_5 = tf.nn.relu((tf.layers.batch_normalization(hidden_8_drop + hidden_7_drop + hidden_6_drop + hidden_5_drop,
                                                                       training=training)/4 + b1))
                hidden_1_5_drop = tf.nn.dropout(hidden_1_5, keep_prob)
                p3 = tf.nn.softmax(tf.layers.batch_normalization(tf.matmul(hidden_1_5_drop, w0)) + b0)
            with tf.name_scope('p4'):
                hidden_1_4 = tf.nn.relu(
                    (tf.layers.batch_normalization(hidden_8_drop + hidden_7_drop + hidden_6_drop + hidden_5_drop + hidden_4_drop,
                                                   training=training)/5 + b1))
                hidden_1_4_drop = tf.nn.dropout(hidden_1_4, keep_prob)
                p4 = tf.nn.softmax(tf.layers.batch_normalization(tf.matmul(hidden_1_4_drop, w0)) + b0)
            with tf.name_scope('p5'):
                hidden_1_3 = tf.nn.relu(
                    (tf.layers.batch_normalization(hidden_8_drop + hidden_7_drop + hidden_6_drop + hidden_5_drop + hidden_4_drop + hidden_3_drop,
                                                   training=training)/6 + b1))
                hidden_1_3_drop = tf.nn.dropout(hidden_1_3, keep_prob)
                p5 = tf.nn.softmax(tf.layers.batch_normalization(tf.matmul(hidden_1_3_drop, w0)) + b0)
            with tf.name_scope('p6'):
                hidden_1_2 = tf.nn.relu((tf.layers.batch_normalization(
                    hidden_8_drop + hidden_7_drop + hidden_6_drop + hidden_5_drop + hidden_4_drop + hidden_3_drop + hidden_2_drop,
                    training=training)/7 + b1))
                hidden_1_2_drop = tf.nn.dropout(hidden_1_2, keep_prob)
                p6 = tf.nn.softmax(tf.layers.batch_normalization(tf.matmul(hidden_1_2_drop, w0)) + b0)
        ...
        with tf.name_scope('train_step0'):
            ...
            with tf.control_dependencies(update_ops):
                ...
                """
                train_step0 = tf.train.AdamOptimizer().minimize(loss0, var_list=[w0, b0, b1, w8, b8])
                train_step1 = tf.train.AdamOptimizer().minimize(loss1, var_list=[w0, b0, b1, w7, b7])
                train_step2 = tf.train.AdamOptimizer().minimize(loss2, var_list=[w0, b0, b1, w6, b6])
                train_step3 = tf.train.AdamOptimizer().minimize(loss3, var_list=[w0, b0, b1, w5, b5])
                train_step4 = tf.train.AdamOptimizer().minimize(loss4, var_list=[w0, b0, b1, w4, b4])
                train_step5 = tf.train.AdamOptimizer().minimize(loss5, var_list=[w0, b0, b1, w3, b3])
                train_step6 = tf.train.AdamOptimizer().minimize(loss6, var_list=[w0, b0, b1, w2, b2])
                """
    ...
    def prepare_session(self):
        ...
        writer = tf.summary.FileWriter("./log/1219_nonliner_dropout", sess.graph)
...
if __name__ == '__main__':
    ...
    drop_out_rate = 1.0
    session_name = "./session_log/saver_1219_nonliner_dropout_6"
    training = True
    testing = False
