        ...
        with tf.name_scope('layer8'):
            W_conv8 = tf.Variable(
                tf.random_uniform(shape=[3, 3, input_channel, num_units8],
                                  minval=-np.sqrt(6.0 / (image_size + 3 * 3 * input_channel * num_units8)),
                                  maxval=np.sqrt(6.0 / (image_size + 3 * 3 * input_channel * num_units8)),
                                  dtype=tf.float32))
...
#             W_conv8 = tf.Variable(tf.truncated_normal([3, 3, input_channel, num_units8], stddev=0.01))
        ...
        with tf.name_scope('layer7'):
            W_conv7 = tf.Variable(
                tf.random_uniform(shape=[3, 3, num_units8, num_units7],
                                  minval=-np.sqrt(6.0 / (image_size + 3*3*num_units8*num_units7)),
                                  maxval=np.sqrt(6.0 / (image_size + 3*3*num_units8*num_units7)),
                                  dtype=tf.float32))
        ...
        with tf.name_scope('layer6'):
            W_conv6 = tf.Variable(
                tf.random_uniform(shape=[3, 3, num_units8, num_units7],
                                  minval=-np.sqrt(6.0 / (image_size + 3 * 3 * num_units7 * num_units6)),
                                  maxval=np.sqrt(6.0 / (image_size + 3 * 3 * num_units7 * num_units6)),
                                  dtype=tf.float32))
        ...
        with tf.name_scope('optimizer'):
            ...
            with tf.name_scope('train_step0'):
                ...
                train_step2 = tf.train.AdamOptimizer().minimize(loss2)
            # Adam
            train_step = [train_step0, train_step1, train_step2]
            ...
            with tf.name_scope('accuracy'):
                ...
                accuracy = [accuracy0, accuracy1, accuracy2]
    ...
    def prepare_session(self):
        ...
        writer = tf.summary.FileWriter("./log/1211_MLP_5layer_CNN", sess.graph)
...
if __name__ == '__main__':
    ...
    session_name = "./session_log/saver_1211_MLP_5layer_CNN_"
