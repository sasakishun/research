...
class layer:
    ...
    def prepare_model(self):
        ...
        with tf.name_scope('feed_forword'):
            ...
            """
            p0 = tf.nn.softmax(tf.matmul(hidden_5, w0_5) + b0_5)#tf.nn.softmax(tf.matmul(hidden_8, w0_8) + b0_8)
            p1 = tf.nn.softmax(tf.matmul(hidden_5, w0_5) + b0_5)#tf.nn.softmax(tf.matmul(hidden_7, w0_7) + b0_7)
            p2 = tf.nn.softmax(tf.matmul(hidden_5, w0_5) + b0_5)#tf.nn.softmax(tf.matmul(hidden_6, w0_6) + b0_6)
        ...
        with tf.name_scope('train_step0'):
            ...
            with tf.control_dependencies(update_ops):
                ...
                train_step0_under = tf.train.AdamOptimizer().minimize(loss0, var_list=[w8, b8, w7, b7, w6, b6, w5, b5, w0_5, b0])
                train_step1_under = tf.train.AdamOptimizer().minimize(loss1, var_list=[w7, b7, w6, b6, w5, b5, w0_5, b0])
                train_step2_under = tf.train.AdamOptimizer().minimize(loss2, var_list=[w6, b6, w5, b5, w0_5, b0])
                train_step3_under = tf.train.AdamOptimizer().minimize(loss3, var_list=[w5, b5, w0_5, b0])
                train_step4_under = tf.train.AdamOptimizer().minimize(loss4, var_list=[w4, b4, w3, b3])
                train_step5_under = tf.train.AdamOptimizer().minimize(loss5, var_list=[w3, b3, w2, b2])
                train_step6_under = tf.train.AdamOptimizer().minimize(loss6, var_list=[w2, b2])
...
if __name__ == '__main__':
    ...
    best_loss = 10000.
    loop_count = 0
    for looping in range(layer_num):
        final_loss_layer = int(looping)
        print(final_loss_layer)
        if final_loss_layer == layer_num-1:
            early_stopping_num = final_early_stopping_num
        ...
        if final_loss_layer != layer_num-1:
            for reset_num in range(early_stopping_num):
                graph_y[loop_count + 1 + reset_num] = None
        final_loss_layer += 1
    ...
    if dataset_flag == 0:
        np.save("GFNN_mnist", graph_y)
    if dataset_flag == 1:
        np.save("GFNN_fashion", graph_y)
    if dataset_flag == 2:
        np.save("GFNN_cifar10", graph_y)
    if dataset_flag == 3:
        np.save("GFNN_cifar100", graph_y)
