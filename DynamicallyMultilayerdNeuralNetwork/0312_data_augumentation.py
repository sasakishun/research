...
class layer:
    ...
    def prepare_model(self):
        ...
        with tf.name_scope('node_size'):
            with tf.name_scope('hidden_unit'):
            ...
            with tf.name_scope('num_units8'):
                ...
                num_units_inner = inner_unit_num  # 100
        ...
        with tf.name_scope('feed_forword'):
            ...
            """
            p0 = tf.nn.softmax(tf.layers.batch_normalization(tf.matmul(hidden_inner_8, w0_inner_8) + b0_8, training=training))
            p1 = tf.nn.softmax(tf.layers.batch_normalization(tf.matmul(hidden_inner_7, w0_inner_7) + b0_7, training=training))
            p2 = tf.nn.softmax(tf.layers.batch_normalization(tf.matmul(hidden_inner_6, w0_inner_6) + b0_6, training=training))
            p3 = tf.nn.softmax(tf.layers.batch_normalization(tf.matmul(hidden_inner_5, w0_inner_5) + b0_5, training=training))
            p4 = tf.nn.softmax(tf.layers.batch_normalization(tf.matmul(hidden_inner_4, w0_inner_4) + b0_4, training=training))
            p5 = tf.nn.softmax(tf.layers.batch_normalization(tf.matmul(hidden_inner_3, w0_inner_3) + b0_3, training=training))
            p6 = tf.nn.softmax(tf.layers.batch_normalization(tf.matmul(hidden_inner_2, w0_inner_2) + b0_2, training=training))
            """
        ...
        with tf.name_scope('train_step0'):
            ...
            with tf.control_dependencies(update_ops):
                ...
                train_step0_under = tf.train.AdamOptimizer().minimize(loss0,
                                                                      var_list=[w_inner_8, b8_inner])
                train_step1_under = tf.train.AdamOptimizer().minimize(loss1,
                                                                      var_list=[w_inner_7, b7_inner])
                train_step2_under = tf.train.AdamOptimizer().minimize(loss2,
                                                                      var_list=[w_inner_6, b6_inner])
                train_step3_under = tf.train.AdamOptimizer().minimize(loss3,
                                                                      var_list=[w_inner_5, b5_inner])
                train_step4_under = tf.train.AdamOptimizer().minimize(loss4,
                                                                      var_list=[w_inner_4, b4_inner])
                train_step5_under = tf.train.AdamOptimizer().minimize(loss5,
                                                                      var_list=[w_inner_3, b3_inner])
                train_step6_under = tf.train.AdamOptimizer().minimize(loss6,
                                                                      var_list=[w_inner_2, b2_inner])
                """
...
if __name__ == '__main__':
    ...
    width = int(np.sqrt(image_size))
    data = data.reshape([train_size, height, width, input_channel])
    data_flip = data[:, :, ::-1, :]
    print(data_flip.shape)

    padded = np.pad(data, ((0, 0), (4, 4), (4, 4), (0, 0)), mode='constant')
    rng = np.random.RandomState(123)
    crops = rng.randint(8, size=(len(data), 2))
    cropped_train_X = [padded[i, c[0]:(c[0] + height), c[1]:(c[1] + width), :] for i, c in enumerate(crops)]
    data_cropped = np.array(cropped_train_X)
    print(data_cropped.shape)

    data_noise = data * rng.binomial(size=data.shape, n=1, p=0.8)
    print(data_noise.shape)

    data_processing = [data_flip.reshape([train_size, height * width * input_channel]),
                       data_cropped.reshape([train_size, height * width * input_channel]),
                       data_noise.reshape([train_size, height * width * input_channel])]
    data = data.reshape([train_size, height * width * input_channel])
    for train_processing in data_processing:
        data = np.concatenate((data, train_processing), axis=0)
    ...
    for i in range(loop_count + 1):
        graph_y[final_loss_layer + 1][i] = None
        graph_train_y[final_loss_layer + 1][i] = None
    print("")
    ...
    summary, loss_val_train, acc_val_train = nn.sess.run(
        ...
        feed_dict={nn.x: train_test_data, nn.t: train_test_labels, nn.keep_prob: 1.0, nn.training: testing,
                   nn.input_batch_size: len(train_test_labels)})
    print('Step(%d): %d, Loss(tr): %f, Accuracy: %f' % (final_loss_layer, loop_count, loss_val_train, acc_val_train))
    summary, loss_val_valid, acc_val_valid = nn.sess.run(
        [nn.summary, nn.loss[final_loss_layer], nn.accuracy[final_loss_layer]],
        feed_dict={nn.x: valid_data, nn.t: valid_labels, nn.keep_prob: 1.0, nn.training: testing,
                   nn.input_batch_size: len(valid_labels)})
    print('Step(%d): %d, Loss(va): %f, Accuracy: %f' % (final_loss_layer, loop_count, loss_val_valid, acc_val_valid))
    summary, loss_val, acc_val = nn.sess.run(
        [nn.summary, nn.loss[final_loss_layer], nn.accuracy[final_loss_layer]],
        feed_dict={nn.x: test_data, nn.t: test_labels, nn.keep_prob: 1.0, nn.training: testing,
                   nn.input_batch_size: len(test_labels)})
    ...
    """
    result is written in 0206.txt
    """
    f = open("0312_log_data_augumentation.txt", 'a')
    f.write("%s, batchsize:%d, train_size:%d \n" % (args[1], batchsize, train_size))
    f.write('Step(%d): %d, Loss(te): %f, Accuracy: %f \n' % (final_loss_layer, loop_count, loss_val, acc_val))
    f.close()
    ...
    for i in range(early_stopping_num):
        # graph_y[final_loss_layer][loop_count + i] = None
        graph_train_y[final_loss_layer][loop_count + i] = None

    ax = plt.subplot()
    print("final_loss_layer: {0}".format(final_loss_layer))
    for i in range(layer_num + 1):
        if i <= final_loss_layer:
            if i == 0:
                ax.plot(graph_x, graph_y[i], linewidth=1.0, marker=".", markersize=10, label="中間層数:2")
            if i == 1:
                ax.plot(graph_x, graph_y[i], linewidth=1.0, marker=".", markersize=10, label="中間層数:3")
            if i == 2:
                ax.plot(graph_x, graph_y[i], linewidth=1.0, marker=".", markersize=10, label="中間層数:4")
            if i == 3:
                ax.plot(graph_x, graph_y[i], linewidth=1.0, marker=".", markersize=10, label="中間層数:5")
            if i == 4:
                ax.plot(graph_x, graph_y[i], linewidth=1.0, marker=".", markersize=10, label="中間層数:6")
            if i == 5:
                ax.plot(graph_x, graph_y[i], linewidth=1.0, marker=".", markersize=10, label="中間層数:7")
            if i == 6:
                ax.plot(graph_x, graph_y[i], linewidth=1.0, marker=".", markersize=10, label="中間層数:8")
    ax.grid(which="both")
    ax.set_xlabel("エポック数", fontsize=15)
    ax.set_ylabel("正答率", fontsize=15)
    # plt.title(str(train_size) + dataset_name + ":" + path.splitext(path.basename(__file__))[0])
    ax.set_xlim([0, loop_count - 1])

    np.save("proposal_y", graph_y)
    if dataset_flag == 0:
        np.save("proposal_mnist", graph_y)
    if dataset_flag == 1:
        np.save("proposal_fashion", graph_y)
    if dataset_flag == 2:
        np.save("proposal_cifar10", graph_y)
    if dataset_flag == 3:
        np.save("proposal_cifar100", graph_y)
    ...
    for i in range(layer_num + 1):
        if i <= final_loss_layer:
            ax.plot(graph_x, graph_y[i], linewidth=1.0, marker=".", markersize=10, color="Blue")
    plt.legend()
    # plt.show()

    data_SAE = np.load("SAE_y.npy")
    print(data_SAE.shape)
    graph_tmp = [None for i in range(1000)]
    ax = plt.subplot()
    ax.plot(graph_x, data_SAE, linewidth=1.0, marker=",", markersize=10, label="SAE", color="Blue")

    data_GSD = np.load("GSD_y.npy")
    ...
    ax.plot(graph_x, graph_tmp_GSD, linewidth=1.0, marker=".", markersize=10, label="GSD", color="Yellow")

    for i in range(final_loss_layer + 1):
        for j in range(len(graph_y[i])):
            if graph_y[i][j] is not None:
                graph_tmp[j] = graph_y[i][j]

    ax.plot(graph_x, graph_tmp, linewidth=1.0, marker=".", markersize=10, label="提案手法", color="Red")
    ax.grid(which="both")
    ax.set_xlabel("エポック数", fontsize=15)
    ax.set_ylabel("正答率", fontsize=15)
    plt.title(str(train_size) + dataset_name + ":" + path.splitext(path.basename(__file__))[0])
