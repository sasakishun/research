...
class layer:
    ...
    def prepare_model(self):
        ...
        with tf.name_scope('feed_forword'):
            ...
            h_pool_7 = tf.nn.max_pool(tf.nn.relu(h_conv7 + b_conv7 +
                                                 h_conv8)
                                      , ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding='SAME')
            h_pool_flat_7 = tf.reshape(h_pool_7, [-1, int((image_size / pooling_down) * num_units0)])

            h_pool_6 = tf.nn.max_pool(tf.nn.relu(h_conv6 + b_conv6 +
                                                 h_conv7 +
                                                 h_conv8)
                                      , ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding='SAME')
        ...
        with tf.name_scope('optimizer'):
            ...
            with tf.name_scope('train_step0'):
                """
                train_step0 = tf.train.AdamOptimizer().minimize(loss0,
                                                                var_list=[W_conv8, b_conv8, w1, b1, w0, b0])
                train_step1 = tf.train.AdamOptimizer().minimize(loss1,
                                                                var_list=[W_conv7, b_conv7, w1, b1, w0, b0])
                train_step2 = tf.train.AdamOptimizer().minimize(loss2,
                                                                var_list=[W_conv6, b_conv6, w1, b1, w0, b0])
                """
                train_step0 = tf.train.AdamOptimizer().minimize(loss0)
                train_step1 = tf.train.AdamOptimizer().minimize(loss1)
                train_step2 = tf.train.AdamOptimizer().minimize(loss2)
    ...
    def prepare_session(self):
        ...
        writer = tf.summary.FileWriter("./log/1210_nonliner_CNN_5layer", sess.graph)
...
if __name__ == '__main__':
    ...
    session_name = "./session_log/saver_1210_nonliner_CNN_5layer"
