    ...
    def prepare_model(self):
        ...
        with tf.name_scope('layer3_output_fully_connected'):
            ...
            with tf.name_scope('w3_0'):
                ...
                # w3_0 = tf.Variable(tf.truncated_normal(shape=[input_size, 10], mean=0.0, stddev=1 / input_size), trainable=False)
    ...
    def prepare_model(self):
        ...
        with tf.name_scope('layer2_fully_connected'):
            ...
            with tf.name_scope('w2'):
                ...
                # w2 = tf.random_uniform(shape=[num_units1, num_units2], minval=-0.0, maxval=0.0, dtype=tf.float32)
            with tf.name_scope('w2_0'):
                w2_0 = tf.Variable(tf.truncated_normal(shape=[input_size, num_units2], mean=0.0, stddev=1 / input_size))
                # w2_0 = tf.Variable(tf.truncated_normal(shape=[input_size, num_units2], mean=0.0, stddev=1 / input_size), trainable=False)
                # w2_0 = tf.placeholder(tf.float32, ([input_size, num_units2]))
            with tf.name_scope('b2'):
                b2 = tf.Variable(tf.constant(0.1, shape=[num_units2]))
            with tf.name_scope('hidden2'):
                hidden2 = tf.nn.relu(tf.matmul(hidden1_drop, w2)
                                     + tf.matmul(x, w2_0)
                                     + b2)
            with tf.name_scope('hidden2_drop'):
                hidden2_drop = tf.nn.dropout(hidden2, keep_prob)
        ...
        with tf.name_scope('layer3_output_fully_connected'):
            ...
            with tf.name_scope('w3_0'):
                ...
                # w3_0 = tf.Variable(tf.zeros([input_size, 10]), trainable=False)
                # w3_0 = tf.placeholder(tf.float32, ([input_size, 10]))
            with tf.name_scope('b3'):
                b3 = tf.Variable(tf.zeros([10]))
            with tf.name_scope('p'):
...
if __name__ == '__main__':
    ...
    nn = layer2()
    # input_w3_0, w2_0, w3_from_layer1
