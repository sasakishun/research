    ...
    def prepare_model(self):
        ...
        with tf.name_scope('optimizer'):
            ...
            with tf.name_scope('loss1'):
                loss1 = -tf.reduce_sum(t * tf.log(tf.clip_by_value(p1, 1e-10, 1.0))) - tf.reduce_sum(
                    gate_9) - tf.reduce_sum(gate_8)
            with tf.name_scope('loss2'):
                loss2 = -tf.reduce_sum(t * tf.log(
                    tf.clip_by_value(p2, 1e-10, 1.0))) - tf.reduce_sum(gate_9) - tf.reduce_sum(gate_8) - tf.reduce_sum(
                    gate_7)  # - tf.reduce_sum(tf.log(tf.clip_by_value(w0_9, 1e-10, 1.0)))
            with tf.name_scope('loss3'):
                loss3 = -tf.reduce_sum(t * tf.log(
                    tf.clip_by_value(p3, 1e-10, 1.0))) - tf.reduce_sum(gate_9) - tf.reduce_sum(gate_8) - tf.reduce_sum(
                    gate_7) - tf.reduce_sum(gate_6)  # - tf.reduce_sum(tf.log(tf.clip_by_value(w0_8, 1e-10, 1.0)))
        ...
        for i in range(loop_len):
            ...
            if j == 0:
                nn.sess.run(nn.train_step[j], feed_dict={nn.x: batch_xs, nn.t: batch_ts, nn.keep_prob: drop_out_rate,
                                                         nn.gate_9: 1.0, nn.gate_8: 0.0, nn.gate_7: 0.0,
                                                         nn.gate_6: 0.0})
            if j == 1:
                nn.sess.run(nn.train_step[j], feed_dict={nn.x: batch_xs, nn.t: batch_ts, nn.keep_prob: drop_out_rate,
                                                         nn.gate_9: 0.0, nn.gate_8: 1.0, nn.gate_7: 0.0,
                                                         nn.gate_6: 0.0})
            if j == 2:
                nn.sess.run(nn.train_step[j], feed_dict={nn.x: batch_xs, nn.t: batch_ts, nn.keep_prob: drop_out_rate,
                                                         nn.gate_9: 0.0, nn.gate_8: 0.0, nn.gate_7: 1.0,
                                                         nn.gate_6: 0.0})
            if j == 3:
                nn.sess.run(nn.train_step[j], feed_dict={nn.x: batch_xs, nn.t: batch_ts, nn.keep_prob: drop_out_rate,
                                                         nn.gate_9: 0.0, nn.gate_8: 0.0, nn.gate_7: 0.0,
                                                         nn.gate_6: 1.0})
