...
class layer:
    ...
    def prepare_session(self):
        ...
        writer = tf.summary.FileWriter("./log/1129_cifar-10_gate_finetuning_with_valid", sess.graph)
...
if __name__ == '__main__':
    ...
    batchsize = 100
    train_size = 45000  # 10000 is good
    early_stopping_num = 10000
    session_name = "./session_log/saver_1129_cifar-10_gate_fine_tuning_with_valid_100"
    ...
    valid_data = np.load("cifar-10_train_data_normalized.npy")[train_size:train_size + int(train_size / 10)]
    valid_labels = np.load("cifar-10_train_labels.npy")[train_size:train_size + int(train_size / 10)]
    ...
    for j in range(5):
        ...
        if acc_val < 0.15:
            if j == 0:
                nn.sess.run(nn.gate_9, feed_dict={nn.gate_9: [0]})
            if j == 1:
                nn.sess.run(nn.gate_8, feed_dict={nn.gate_8: [0]})
            if j == 2:
                nn.sess.run(nn.gate_7, feed_dict={nn.gate_7: [0]})
