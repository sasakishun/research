...
class layer:
    ...
    def prepare_model(self):
        ...
        with tf.name_scope('layer0'):
            ...
            with tf.name_scope('b0'):
                ...
                gate_9 = tf.Variable(tf.constant(1., shape=[input_size, output_size]))
                gate_8 = tf.Variable(tf.constant(1., shape=[num_units8, output_size]))
                gate_7 = tf.Variable(tf.constant(1., shape=[num_units7, output_size]))
                gate_6 = tf.Variable(tf.constant(1., shape=[num_units6, output_size]))
        ...
        with tf.name_scope('feed_forword'):
            with tf.name_scope('state0'):
                with tf.name_scope('p0'):
                    p0 = tf.nn.softmax(tf.matmul(x, w0_9 * tf.sigmoid(gate_9)) + b0)
            ...
            with tf.name_scope('state1'):
                ...
                with tf.name_scope('p1'):
                    p1 = tf.nn.softmax(tf.matmul(hidden1_8_drop, (w0_8 * tf.sigmoid(gate_8)))
                                       + b0
                                       + tf.matmul(x, (w0_9 * tf.sigmoid(gate_9))))
            ...
            with tf.name_scope('state2'):
                ...
                with tf.name_scope('p2'):
                    p2 = tf.nn.softmax(tf.matmul(hidden2_7_drop, w0_7 * tf.sigmoid(gate_7))
                                       + b0
                                       + tf.matmul(x, w0_9 * tf.sigmoid(gate_9))
                                       + tf.matmul(hidden2_8, w0_8 * tf.sigmoid(gate_8)))
            ...
            with tf.name_scope('state3'):
                ...
                with tf.name_scope('p3'):
                    p3 = tf.nn.softmax(tf.matmul(x, w0_9 * tf.sigmoid(gate_9))
                                       + b0
                                       + tf.matmul(hidden3_6_drop, w0_6 * tf.sigmoid(gate_6))
                                       + tf.matmul(hidden3_7_drop, w0_7 * tf.sigmoid(gate_7))
                                       + tf.matmul(hidden3_8_drop, w0_8 * tf.sigmoid(gate_8)))
            ...
            with tf.name_scope('state4'):
                ...
                with tf.name_scope('p4'):
                    p4 = tf.nn.softmax(tf.matmul(x, w0_9 * tf.sigmoid(gate_9))
                                       + b0
                                       + tf.matmul(hidden4_6_drop, w0_6 * tf.sigmoid(gate_6))
                                       + tf.matmul(hidden4_7_drop, w0_7 * tf.sigmoid(gate_7))
                                       + tf.matmul(hidden4_8_drop, w0_8 * tf.sigmoid(gate_8)))
    ...
    def prepare_session(self):
        ...
        writer = tf.summary.FileWriter("./log/1207_MNIST_nonliner_highway", sess.graph)
...
if __name__ == '__main__':
    ...
    session_name = "./session_log/saver_1207_MNIST_nonliner_highway_2"
    ...
    for j in range(layer_num + 1):
        ...
        """
        if j == 1:
            tmp_array = nn.sess.run(nn.gate_9)
            nn.sess.run(nn.gate_9, feed_dict={nn.gate_9: tmp_array * 0.})
        if j == 2:
            tmp_array = nn.sess.run(nn.gate_9)
            nn.sess.run(nn.gate_9, feed_dict={nn.gate_9: tmp_array * 0.})
            tmp_array = nn.sess.run(nn.gate_8)
            nn.sess.run(nn.gate_8, feed_dict={nn.gate_8: tmp_array * 0.})
        if j == 3:
            tmp_array = nn.sess.run(nn.gate_9)
            nn.sess.run(nn.gate_9, feed_dict={nn.gate_9: tmp_array * 0.})
            tmp_array = nn.sess.run(nn.gate_8)
            nn.sess.run(nn.gate_8, feed_dict={nn.gate_8: tmp_array * 0.})
            tmp_array = nn.sess.run(nn.gate_7)
            nn.sess.run(nn.gate_7, feed_dict={nn.gate_7: tmp_array * 0.})
        """
        """
        if j == 1:
        ...
        if j == 2:
            ...
            nn.sess.run(nn.w0_9, feed_dict={nn.w0_9: tmp_array * np.sqrt(
                np.sqrt((nn.input_size + nn.hidden_unit + nn.output_size)
                        / (nn.input_size + nn.hidden_unit * 2 + nn.output_size)))})
        ...
        if j == 3:
            ...
            nn.sess.run(nn.w0_9, feed_dict={nn.w0_9: tmp_array * np.sqrt(
                np.sqrt((nn.input_size + nn.hidden_unit*2 + nn.output_size)
                        / (nn.input_size + nn.hidden_unit * 3 + nn.output_size)))})
            tmp_array = nn.sess.run(nn.w0_8)
            nn.sess.run(nn.w0_8, feed_dict={nn.w0_8: tmp_array * np.sqrt(
                np.sqrt((nn.input_size + nn.hidden_unit*2 + nn.output_size)
                        / (nn.input_size + nn.hidden_unit * 3 + nn.output_size)))})
