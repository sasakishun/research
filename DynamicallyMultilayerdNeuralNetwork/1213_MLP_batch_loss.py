...
class layer:
    ...
    def prepare_model(self):
        ...
        with tf.name_scope('feed_forword'):
            ...
            with tf.name_scope('p0'):
                p0 = tf.nn.softmax(tf.layers.batch_normalization(tf.matmul(hidden_8, w0_8), training=training) + b0)
            with tf.name_scope('p1'):
                p1 = tf.nn.softmax(tf.layers.batch_normalization(tf.matmul(hidden_7, w0_7), training=training) + b0)
            with tf.name_scope('p2'):
                p2 = tf.nn.softmax(tf.layers.batch_normalization(tf.matmul(hidden_6, w0_6), training=training) + b0)
            with tf.name_scope('p3'):
                p3 = tf.nn.softmax(tf.layers.batch_normalization(tf.matmul(hidden_5, w0_5), training=training) + b0)
            with tf.name_scope('p4'):
                p4 = tf.nn.softmax(tf.layers.batch_normalization(tf.matmul(hidden_4, w0_4), training=training) + b0)
            with tf.name_scope('p5'):
                p5 = tf.nn.softmax(tf.layers.batch_normalization(tf.matmul(hidden_3, w0_3), training=training) + b0)
            with tf.name_scope('p6'):
                p6 = tf.nn.softmax(tf.layers.batch_normalization(tf.matmul(hidden_2, w0_2), training=training) + b0)
    ...
    def prepare_session(self):
        ...
        writer = tf.summary.FileWriter("./log/1213_MLP_batch_loss", sess.graph)
...
if __name__ == '__main__':
    ...
    session_name = "./session_log/saver_1213_MLP_batch_loss_2"
    training = True
    testing = False
