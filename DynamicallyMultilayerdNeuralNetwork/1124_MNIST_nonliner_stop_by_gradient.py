...
if __name__ == '__main__':
    ...
    session_name = "saver_1122_MNIST_nonliner_55000"
    past_loss = 10000.
    tmp_loss = 10000.
    ...
    for j in range(4):
        ...
        for i in range(loop_len):
            ...
            tmp_loss = nn.sess.run(nn.loss[j], feed_dict={nn.x: batch_xs, nn.t: batch_ts, nn.keep_prob: drop_out_rate,
                                                          ...
                                                          nn.gate_7: gate_value[2],
                                                          nn.gate_6: gate_value[3]})
            if abs((past_loss - tmp_loss)/past_loss) < 0.00001:
                nn.writer.add_summary(summary, loop_count)
                print("early_stopping is trigger at step:{0}".format(loop_count))
                break
            past_loss = tmp_loss
            if i % 100 == 0:
                # train_data
