...
class layer:
    ...
    def prepare_model(self):
        ...
        with tf.name_scope('feed_forword'):
            ...
            with tf.name_scope('p0'):
                p0 = tf.nn.softmax(tf.layers.batch_normalization(tf.matmul(hidden_8, w0), training=training) + b0)# + b0_8)
            with tf.name_scope('p1'):
                p1 = tf.nn.softmax(tf.layers.batch_normalization(tf.matmul(hidden_7, w0), training=training) + b0)# + b0_7)
            with tf.name_scope('p2'):
                p2 = tf.nn.softmax(tf.layers.batch_normalization(tf.matmul(hidden_6, w0), training=training) + b0)# + b0_6)
            with tf.name_scope('p3'):
                p3 = tf.nn.softmax(tf.layers.batch_normalization(tf.matmul(hidden_5, w0), training=training) + b0)# + b0_5)
            with tf.name_scope('p4'):
                p4 = tf.nn.softmax(tf.layers.batch_normalization(tf.matmul(hidden_4, w0), training=training) + b0)# + b0_4)
            with tf.name_scope('p5'):
                p5 = tf.nn.softmax(tf.layers.batch_normalization(tf.matmul(hidden_3, w0), training=training) + b0)# + b0_3)
            with tf.name_scope('p6'):
                p6 = tf.nn.softmax(tf.layers.batch_normalization(tf.matmul(hidden_2, w0), training=training) + b0)# + b0_2)
        ...
        with tf.name_scope('train_step0'):
            ...
            with tf.control_dependencies(update_ops):
                train_step0 = tf.train.AdamOptimizer().minimize(loss0, var_list=[w0_8, b0, w8, b8])
                train_step1 = tf.train.AdamOptimizer().minimize(loss1, var_list=[w0_7, b0, w7, b7])
                train_step2 = tf.train.AdamOptimizer().minimize(loss2, var_list=[w0_6, b0, w6, b6])
                train_step3 = tf.train.AdamOptimizer().minimize(loss3, var_list=[w0_5, b0, w5, b5])
                train_step4 = tf.train.AdamOptimizer().minimize(loss4, var_list=[w0_4, b0, w4, b4])
                train_step5 = tf.train.AdamOptimizer().minimize(loss5, var_list=[w0_3, b0, w3, b3])
                train_step6 = tf.train.AdamOptimizer().minimize(loss6, var_list=[w0_2, b0, w2, b2])
                """
    ...
    def prepare_session(self):
        ...
        writer = tf.summary.FileWriter("./log/1213_MLP_under_to_top", sess.graph)
...
if __name__ == '__main__':
    ...
    session_name = "./session_log/saver_1213_MLP_under_to_top_20"
    training = True
    testing = False
    ...
    ax = plt.subplot()
    ax.plot(graph_x, graph_y, linewidth=1.0)
    ax.plot(graph_x, graph_train_y, linewidth=1.0)
    ax.grid(which="both")
    ax.set_xlabel("epoch")
