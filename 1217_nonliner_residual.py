...
class layer:
    ...
    def prepare_model(self):
        ...
        with tf.name_scope('layer2_out'):
            with tf.name_scope('b2'):
                b2_out = tf.Variable(tf.constant(0.1, shape=[num_units2]))

        with tf.name_scope('layer1'):
            with tf.name_scope('w1'):
                w1 = tf.Variable(
                    tf.random_uniform(shape=[num_units2, 1000],
                                      minval=-np.sqrt(6.0 / (num_units2 + 1000)),
                                      maxval=np.sqrt(6.0 / (num_units2 + 1000)),
                                      dtype=tf.float32))
            ...
            with tf.name_scope('w0'):
                w0 = tf.Variable(
                    tf.random_uniform(shape=[1000, output_size],
                                      minval=-np.sqrt(3.0 / (1000 + output_size)),
                                      maxval=np.sqrt(3.0 / (1000 + output_size)),
                                      dtype=tf.float32))
                b0 = tf.Variable(tf.constant(0.0, shape=[output_size]))
        ...
        with tf.name_scope('feed_forword'):
            ...
            with tf.name_scope('p0'):
                hidden_2_8 = tf.nn.relu((tf.layers.batch_normalization(hidden_8, training=training) + b2_out))
                hidden_1_8 = tf.nn.relu(
                    tf.layers.batch_normalization(tf.matmul(hidden_2_8, w1), training=training) + b1)
                p0 = tf.nn.softmax(tf.layers.batch_normalization(tf.matmul(hidden_1_8, w0)) + b0)
            with tf.name_scope('p1'):
                hidden_2_7 = tf.nn.relu(
                    (tf.layers.batch_normalization(hidden_7, training=training) + hidden_8 + b2_out))
                hidden_1_7 = tf.nn.relu(
                    (tf.layers.batch_normalization(tf.matmul(hidden_2_7, w1), training=training) + b1))
                p1 = tf.nn.softmax(tf.layers.batch_normalization(tf.matmul(hidden_1_7, w0)) + b0)
            with tf.name_scope('p2'):
                hidden_2_6 = tf.nn.relu(
                    (tf.layers.batch_normalization(hidden_6, training=training)
                     + hidden_8 + hidden_7 + b2_out))
                hidden_1_6 = tf.nn.relu(
                    (tf.layers.batch_normalization(tf.matmul(hidden_2_6, w1), training=training) + b1))
                p2 = tf.nn.softmax(tf.layers.batch_normalization(tf.matmul(hidden_1_6, w0)) + b0)
            with tf.name_scope('p3'):
                hidden_2_5 = tf.nn.relu(tf.layers.batch_normalization(hidden_5, training=training)
                                        + hidden_8 + hidden_7 + hidden_6 + b2_out)
                hidden_1_5 = tf.nn.relu(
                    tf.layers.batch_normalization(tf.matmul(hidden_2_5, w1), training=training) + b1)
                p3 = tf.nn.softmax(tf.layers.batch_normalization(tf.matmul(hidden_1_5, w0)) + b0)
            with tf.name_scope('p4'):
                hidden_2_4 = tf.nn.relu(
                    tf.layers.batch_normalization(hidden_4, training=training)
                    + hidden_8 + hidden_7 + hidden_6 + hidden_5 + b2_out)
                hidden_1_4 = tf.nn.relu(
                    tf.layers.batch_normalization(tf.matmul(hidden_2_4, w1), training=training) + b1)
                p4 = tf.nn.softmax(tf.layers.batch_normalization(tf.matmul(hidden_1_4, w0)) + b0)
            with tf.name_scope('p5'):
                hidden_2_3 = tf.nn.relu(
                    tf.layers.batch_normalization(hidden_3, training=training)
                    + hidden_8 + hidden_7 + hidden_6 + hidden_5 + hidden_4 + b2_out)
                hidden_1_3 = tf.nn.relu(
                    (tf.layers.batch_normalization(tf.matmul(hidden_2_3, w1), training=training) + b1))
                p5 = tf.nn.softmax(tf.layers.batch_normalization(tf.matmul(hidden_1_3, w0)) + b0)
            with tf.name_scope('p6'):
                hidden_2_2 = tf.nn.relu(
                    tf.layers.batch_normalization(hidden_2, training=training)
                    + hidden_8 + hidden_7 + hidden_6 + hidden_5 + hidden_4 + hidden_3 + b2_out)
                hidden_1_2 = tf.nn.relu(
                    (tf.layers.batch_normalization(tf.matmul(hidden_2_2, w1), training=training) + b1))
                p6 = tf.nn.softmax(tf.layers.batch_normalization(tf.matmul(hidden_1_2, w0)) + b0)
        ...
        with tf.name_scope('train_step0'):
            ...
            with tf.control_dependencies(update_ops):
                train_step0 = tf.train.AdamOptimizer().minimize(loss0,
                                                                var_list=[w0, b0, w1, b1, b2_out, w8, b8])
                train_step1 = tf.train.AdamOptimizer().minimize(loss1, var_list=[w0, b0, w1, b1, b2_out, w7, b7])
                train_step2 = tf.train.AdamOptimizer().minimize(loss2,
                                                                var_list=[w0, b0, w1, b1, b2_out, w6, b6])  # , gate_8])
                train_step3 = tf.train.AdamOptimizer().minimize(loss3,
                                                                var_list=[w0, b0, w1, b1, b2_out, w5, b5])  # , gate_7, gate_8])
                train_step4 = tf.train.AdamOptimizer().minimize(loss4,
                                                                var_list=[w0, b0, w1, b1, b2_out, w4, b4])  # , gate_6, gate_7, gate_8])
                train_step5 = tf.train.AdamOptimizer().minimize(loss5,
                                                                var_list=[w0, b0, w1, b1, b2_out, w3, b3])  # , gate_5, gate_6, gate_7, gate_8])
                train_step6 = tf.train.AdamOptimizer().minimize(loss6,
                                                                var_list=[w0, b0, w1, b1, b2_out, w2, b2])  # , gate_4, gate_5, gate_6, gate_7, gate_8])
                """
        ...
        tf.summary.scalar("gate_5", tf.reduce_sum(gate_5))
        tf.summary.scalar("gate_4", tf.reduce_sum(gate_4))
        tf.summary.scalar("gate_3", tf.reduce_sum(gate_3))
        tf.summary.scalar("gate_2", tf.reduce_sum(gate_2))
    ...
    def prepare_session(self):
        ...
        writer = tf.summary.FileWriter("./log/1217_nonliner_residual", sess.graph)
...
if __name__ == '__main__':
    ...
    session_name = "./session_log/saver_1217_nonliner_residual_4"
    training = True
    testing = False
    ...
    final_loss_layer = 0
    graph_x = range(1000)
    graph_y = [0. for i in range(1000)]
    graph_train_y = [0. for i in range(1000)]
    ...
    for j in range(start, layer_num + 1):
        ...
        for i in range(loop_len):
            ...
            if i % 1 == 0:
                ...
                graph_train_y[loop_count] = acc_val
                train_loss = loss_val
        ...
        print('Step: %d, Loss(tr): %f, Accuracy: %f' % (loop_count, loss_val_valid, acc_val_valid))
        summary, loss_val_valid, acc_val_valid = nn.sess.run(
            [nn.summary, nn.loss[j], nn.accuracy[j]],
            feed_dict={nn.x: valid_data, nn.t: valid_labels, nn.keep_prob: 1.0, nn.training: testing,
                       nn.input_batch_size: len(valid_labels)})
    ...
    for i in range(early_stopping_num):
        graph_y[loop_count + i] = 0
        graph_train_y[loop_count + i] = 0
    ax = plt.subplot()
    ax.plot(graph_x, graph_y, linewidth=0.3)
    ax.plot(graph_x, graph_train_y, linewidth=0.3)
    ax.grid(which="both")
    ax.set_xlabel("epoch")
