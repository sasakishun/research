    ...
    def prepare_model(self):
        ...
        with tf.name_scope('feed_forword'):
            with tf.name_scope('state0'):
                with tf.name_scope('p0'):
                    p0 = tf.nn.softmax(tf.multiply(tf.matmul(x, w0_9), tf.tanh(gate_9 + b0_9))
                                       + b0)
            ...
            with tf.name_scope('state1'):
                ...
                with tf.name_scope('p1'):
                    p1 = tf.nn.softmax(tf.multiply(tf.matmul(hidden1_8_drop, w0_8), tf.tanh(gate_8 + b0_8))
                                       + b0
                                       + tf.multiply(tf.matmul(x, w0_9), tf.tanh(gate_9 + b0_9)))
            ...
            with tf.name_scope('state2'):
                ...
                with tf.name_scope('p2'):
                    p2 = tf.nn.softmax(tf.multiply(tf.matmul(hidden2_7_drop, w0_7), tf.tanh(gate_7 + b0_7))
                                       + b0
                                       + tf.multiply(tf.matmul(x, w0_9), tf.tanh(gate_9 + b0_9))
                                       + tf.multiply(tf.matmul(hidden2_8, w0_8), tf.tanh(gate_8 + b0_8)))
            ...
            with tf.name_scope('state3'):
                ...
                with tf.name_scope('p3'):
                    p3 = tf.nn.softmax(tf.multiply(tf.matmul(x, w0_9), tf.tanh(gate_9 + b0_9))
                                       + b0
                                       + tf.multiply(tf.matmul(hidden3_6_drop, w0_6), tf.tanh(gate_6 + b0_6))
                                       + tf.multiply(tf.matmul(hidden3_7_drop, w0_7), tf.tanh(gate_7 + b0_7))
                                       + tf.multiply(tf.matmul(hidden3_8_drop, w0_8), tf.tanh(gate_8 + b0_8)))
        ...
        with tf.name_scope('optimizer'):
            with tf.name_scope('loss0'):
                loss0 = -tf.reduce_sum(t * tf.log(tf.clip_by_value(p0, 1e-10, 1.0)))-tf.reduce_sum(gate_9)
            with tf.name_scope('loss1'):
                loss1 = -tf.reduce_sum(t * tf.log(tf.clip_by_value(p1, 1e-10, 1.0)))-tf.reduce_sum(gate_9)-tf.reduce_sum(gate_8)
            with tf.name_scope('loss2'):
                loss2 = -tf.reduce_sum(t * tf.log(
                    tf.clip_by_value(p2, 1e-10, 1.0)))-tf.reduce_sum(gate_9)-tf.reduce_sum(gate_8)-tf.reduce_sum(gate_7)  # - tf.reduce_sum(tf.log(tf.clip_by_value(w0_9, 1e-10, 1.0)))
            with tf.name_scope('loss3'):
                loss3 = -tf.reduce_sum(t * tf.log(
                    tf.clip_by_value(p3, 1e-10, 1.0)))-tf.reduce_sum(gate_9)-tf.reduce_sum(gate_8)-tf.reduce_sum(gate_7)-tf.reduce_sum(gate_6)  # - tf.reduce_sum(tf.log(tf.clip_by_value(w0_8, 1e-10, 1.0)))
            ...
            """
            with tf.name_scope('train_step0'):
                train_step0 = tf.train.AdamOptimizer().minimize(loss0, var_list=[w0_9])
            with tf.name_scope('train_step1'):
                train_step1 = tf.train.AdamOptimizer().minimize(loss1, var_list=[w0_8, w8, b8])
            with tf.name_scope('train_step2'):
                train_step2 = tf.train.AdamOptimizer().minimize(loss2, var_list=[w0_7, w7, b7])
            with tf.name_scope('train_step3'):
                train_step3 = tf.train.AdamOptimizer().minimize(loss3, var_list=[w0_6, w6, b6])
            with tf.name_scope('train_step4'):
                train_step4 = tf.train.AdamOptimizer().minimize(loss4, var_list=[w0_5, w5, b5])
            with tf.name_scope('train_step5'):
                train_step5 = tf.train.AdamOptimizer().minimize(loss5, var_list=[w0_4, w4, b4])
            with tf.name_scope('train_step6'):
                train_step6 = tf.train.AdamOptimizer().minimize(loss6, var_list=[w0_3, w3, b3])
            with tf.name_scope('train_step7'):
                train_step7 = tf.train.AdamOptimizer().minimize(loss7, var_list=[w0_2, w2, b2])
            with tf.name_scope('train_step8'):
                train_step8 = tf.train.AdamOptimizer().minimize(loss8, var_list=[w0, b0, w1, b1])
            """
...
if __name__ == '__main__':
    data = np.load("MNIST_train_data.npy")[0:40000]
    valid_data = np.load("MNIST_train_data.npy")[40000:55000]
    labels = np.load("MNIST_train_labels.npy")[0:40000]
    valid_labels = np.load("MNIST_train_labels.npy")[40000:55000]
    ...
    print(data.shape)
    train_size = 20000  # 10000 is good
